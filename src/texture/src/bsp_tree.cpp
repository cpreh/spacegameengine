...
sge::bsp_tree::bsp_tree(const dim_type dim)
 : head(value_type(point_type(0,0),dim), 0, false)
{}

sge::bsp_tree::iterator sge::bsp_tree::insert(const dim_type dim)
{
	return insert_recursive(dim, head);
}

void sge::bsp_tree::erase(const iterator it)
{
	delete it.ref;
}

sge::bsp_tree::iterator sge::bsp_tree::find(const value_type& t)
{
	return find_recursive(t, head);
}

sge::bsp_tree::iterator sge::bsp_tree::end()
{
	return iterator(0);
}
...
sge::bsp_tree::iterator sge::bsp_tree::insert_case_2(const dim_type dim, node& ref, node& exist, node*& empty)
{
...
sge::bsp_tree::iterator sge::bsp_tree::insert_node(node& ref, node*& n, const bool final, const value_type& rect)
{
	n = new node(rect, &ref, final);
	return iterator(n);
}

sge::bsp_tree::iterator sge::bsp_tree::insert_two(const dim_type dim, node& ref, node*& n, const value_type& rect)
{
	return insert_recursive(dim, *insert_node(ref, n, false, rect).ref);
}

sge::bsp_tree::iterator sge::bsp_tree::find_recursive(const value_type& t, node& n)
{
	if(n.rect == t)
		return iterator(&n);
