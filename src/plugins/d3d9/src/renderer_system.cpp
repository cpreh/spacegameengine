...
{
	if(!sys)
		throw exception(SGE_TEXT("Initialization of d3d failed!"));
}
...
const sge::renderer_caps_array sge::d3d9::renderer_system::caps() const
{
	...
	for(UINT ad = 0; ad < adapter_count; ++ad)
		v.push_back(create_renderer_caps(ad, sys));
	return v;
}

const sge::renderer_ptr sge::d3d9::renderer_system::create_renderer(
