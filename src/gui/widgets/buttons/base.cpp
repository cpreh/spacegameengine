...
void sge::gui::widgets::buttons::base::process_mouse_enter(events::mouse_enter const &)
{
	mouse_over_ = true;
	parent_manager().dirty(*this,rect(point::null(),size()));
}

void sge::gui::widgets::buttons::base::process_mouse_leave(events::mouse_leave const &)
{
	mouse_over_ = false;
	parent_manager().dirty(*this,rect(point::null(),size()));
}
