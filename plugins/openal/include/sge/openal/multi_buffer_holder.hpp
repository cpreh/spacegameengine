...
{
  FCPPT_NONMOVABLE(multi_buffer_holder);

public:
  using size_type = sge::openal::buffer_id_container::size_type;

  explicit multi_buffer_holder(size_type);

  ~multi_buffer_holder();

  [[nodiscard]] sge::openal::buffer_id_container const &container() const;
