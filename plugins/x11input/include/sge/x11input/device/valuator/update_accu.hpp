...
std::pair<Result, sge::x11input::device::valuator::accu> update_accu(
    sge::x11input::device::valuator::accu _accu,
    sge::x11input::device::valuator::value const _value)
{
  _accu += sge::x11input::device::valuator::accu{_value.get()};

  Result const diff{fcppt::cast::float_to_int<Result>(_accu.get())};

  _accu -= sge::x11input::device::valuator::accu{
      fcppt::cast::int_to_float<sge::x11input::device::valuator::accu::value_type>(diff)};

  return std::make_pair(diff, _accu);
}
