...
using static_type = std::conditional_t<
    sge::x11input::event::is_raw<Value>::value,
    XIRawEvent,
    std::conditional_t<
        sge::x11input::event::is_device<Value>::value,
        XIDeviceEvent,
        std::conditional_t<
            sge::x11input::event::is_hierarchy<Value>::value,
            XIHierarchyEvent,
            fcppt::unit>>>;
