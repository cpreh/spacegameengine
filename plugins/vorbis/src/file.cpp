...
sge::vorbis::file::file(
    ...
    : sge::audio::file(),
      ...
      stdstream_(std::move(_stdstream)),
      stream_(std::move(_stream)),
      info_(sge::vorbis::info(*stream_, name_))
{
}

sge::audio::sample_count sge::vorbis::file::read(
    sge::audio::sample_count const _samples,
    fcppt::reference<sge::audio::sample_buffer> const _buffer)
{
  ...
  sge::audio::sample_count const bytes_to_read{
      _samples * this->channels().get() * bytes_per_sample};

  // TODO(philipp): Make this API less ugly
  sge::audio::sample_buffer::size_type const old_size{_buffer.get().read_size()};

  _buffer.get() = fcppt::container::buffer::append_from(
      std::move(_buffer.get()),
      bytes_to_read,
      [bytes_per_sample, this](
  ...
  return (_buffer.get().read_size() - old_size) / bytes_per_sample;
}

sge::audio::sample_container sge::vorbis::file::read_all()
{
...
sge::audio::channel_count sge::vorbis::file::channels() const
{
  return sge::audio::channel_count{fcppt::cast::to_unsigned(info_.channels)};
}

sge::audio::sample_rate sge::vorbis::file::sample_rate() const
{
  return sge::audio::sample_rate{fcppt::cast::to_unsigned(info_.rate)};
}

sge::audio::bits_per_sample sge::vorbis::file::bits_per_sample() const
{
  return sge::audio::bits_per_sample{
      16U // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  };
}

sge::audio::sample_count sge::vorbis::file::expected_package_size() const
{
  // FIXME: How do we determine the correct size? :(
  return sge::audio::sample_count{
      2048 // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  };
}

void sge::vorbis::file::reset() { sge::vorbis::seek(*stream_, name_, 0); }

sge::vorbis::file::~file() = default;
