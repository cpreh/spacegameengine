...
awl::event::container sge::sdlinput::translate::event(
    ...
    awl::backends::sdl::system::event::object const &_event)
{
  ...
  case SDL_WINDOWEVENT:
    return sge::sdlinput::translate::window_event(_cursor, _focus, _window, _event.get().window);
  case SDL_KEYDOWN:
  case SDL_KEYUP:
    return sge::sdlinput::translate::keyboard_event(_focus, _keyboard, _window, _event.get().key);
  case SDL_TEXTINPUT:
    return sge::sdlinput::focus::translate_text_event(_focus, _window, _event.get().text);
  case SDL_MOUSEMOTION:
    return sge::sdlinput::translate::mouse_motion_event(
        _cursor, _mouse, _window, _event.get().motion);
  case SDL_MOUSEBUTTONDOWN:
  case SDL_MOUSEBUTTONUP:
    return sge::sdlinput::translate::mouse_button_event(
        _cursor, _mouse, _window, _event.get().button);
  case SDL_MOUSEWHEEL:
    return sge::sdlinput::translate::mouse_wheel_event(
        _cursor, _mouse, _window, _event.get().wheel);
  case SDL_JOYAXISMOTION:
    return sge::sdlinput::joypad::translate_axis_event(_joypads.get(), _event.get().jaxis);
  case SDL_JOYBALLMOTION:
    return sge::sdlinput::joypad::translate_ball_event(_joypads.get(), _event.get().jball);
  case SDL_JOYHATMOTION:
    return sge::sdlinput::joypad::translate_hat_event(_joypads.get(), _event.get().jhat);
  case SDL_JOYBUTTONDOWN:
  case SDL_JOYBUTTONUP:
    return sge::sdlinput::joypad::translate_button_event(_joypads.get(), _event.get().jbutton);
  case SDL_JOYDEVICEADDED:
    return sge::sdlinput::joypad::discover(_joypads, _sge_window, _event.get().jdevice);
  case SDL_JOYDEVICEREMOVED:
    return sge::sdlinput::joypad::remove(_joypads, _event.get().jdevice);
