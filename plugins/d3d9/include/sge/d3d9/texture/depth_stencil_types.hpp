...
struct depth_stencil_types
{
  ...
  typedef fcppt::function<D3DLOCKED_RECT(
      d3d_type *, sge::renderer::texture::stage, lock_dest const &, sge::d3d9::lock_flags)>
      lock_function;

  typedef fcppt::function<void(d3d_type *, sge::renderer::texture::stage)> unlock_function;

  typedef fcppt::function<unique_ptr(
      IDirect3DDevice9 *, parameters const &, D3DFORMAT, D3DPOOL, sge::d3d9::usage)>
      create_function;

  static create_function const create();
};
