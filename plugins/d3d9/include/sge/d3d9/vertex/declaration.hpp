...
{
  FCPPT_NONCOPYABLE(declaration);

public:
  declaration(IDirect3DDevice9 &, sge::renderer::vertex::declaration_parameters const &);

  ~declaration();

  IDirect3DVertexDeclaration9 &get() const;

  sge::renderer::vf::dynamic::format const &format() const;

  sge::renderer::vf::dynamic::stride const stride(sge::renderer::vf::dynamic::part_index) const;

  sge::d3d9::vf::texture_coordinate_count const texture_coordinates() const;
