...
sge::d3d9::device::device(
    ...
    : sge::renderer::device::ffp(),
      index_(fcppt::cast::to_unsigned(D3DADAPTER_DEFAULT)),
      srgb_(sge::d3d9::parameters::extract_srgb(_parameters.window().get().awl_object().visual())),
      caps_(sge::d3d9::get_device_caps(_system, index_)),
      present_parameters_(sge::d3d9::parameters::create(
      ...
      device_(sge::d3d9::create_device(_system, index_, present_parameters_)),
      resources_(),
      swap_chain_(sge::d3d9::devicefuncs::get_swap_chain(*device_, 0u)),
      onscreen_target_(fcppt::make_unique_ptr<sge::d3d9::target::onscreen>(
          *device_,
          ...
          sge::renderer::target::viewport(sge::renderer::pixel_rect(
              ...
              fcppt::math::dim::
                  structure_cast<sge::renderer::pixel_rect::dim, fcppt::cast::size_fun>(
                      fcppt::math::dim::to_signed(_parameters.window().get().size())))),
          resources_,
          caps_.target_surfaces())),
      core_defaults_(fcppt::make_unique_ptr<sge::d3d9::state::core::defaults>(*device_)),
      ffp_defaults_(fcppt::make_unique_ptr<sge::d3d9::state::ffp::defaults>(*device_)),
      display_modes_(sge::d3d9::get_display_modes(
          _system,
          index_,
          sge::d3d9::parameters::convert::bit_depth(
              sge::d3d9::parameters::extract_pixel_format(
                  _parameters.window().get().awl_object().visual())
                  .color()))),
      window_{_parameters.window().get()},
      draw_timer_{
          // TODO: How do we know when this changes?
          _parameters.window().get().system().awl_system().processor().create_timer(
              sge::renderer::display_mode::draw_timer_setting_opt(_log, this->display_mode()))},
      window_event_connection_{_parameters.window().get().event_handler(
          sge::window::event_function{[this](awl::window::event::base const &_event)
                                      { return this->on_window_event(_event); }})},
      system_event_connection_{
          _parameters.window().get().system().event_handler(sge::window::system_event_function{
              [this](awl::event::base const &_event) { return this->on_system_event(_event); }})}
{
}
...
sge::d3d9::device::begin_rendering(sge::renderer::target::base_ref const _target)
{
  return fcppt::unique_ptr_to_base<sge::renderer::context::core>(
      this->begin_rendering_ffp(_target));
}

void sge::d3d9::device::end_rendering(sge::renderer::context::core &_context)
{
  sge::d3d9::devicefuncs::end_scene(*device_);

  if (sge::d3d9::render_context::needs_present(_context))
    if (!sge::d3d9::swapchainfuncs::present(*swap_chain_, srgb_))
      this->reset();
}

sge::renderer::target::offscreen_unique_ptr sge::d3d9::device::create_target()
{
  return fcppt::unique_ptr_to_base<sge::renderer::target::offscreen>(
      fcppt::make_unique_ptr<sge::d3d9::target::offscreen>(*device_, caps_.target_surfaces()));
}

sge::renderer::texture::planar_unique_ptr
sge::d3d9::device::create_planar_texture(sge::renderer::texture::planar_parameters const &_param)
{
  return fcppt::unique_ptr_to_base<sge::renderer::texture::planar>(
      this->add_resource(fcppt::make_unique_ptr<sge::d3d9::texture::planar>(*device_, _param)));
}

sge::renderer::texture::depth_stencil_unique_ptr sge::d3d9::device::create_depth_stencil_texture(
    sge::renderer::texture::depth_stencil_parameters const &_params)
{
  // FIXME
  throw sge::renderer::exception{FCPPT_TEXT("Depth stencil textures are not implenented.")};
}
...
{
  return fcppt::unique_ptr_to_base<sge::renderer::depth_stencil_buffer::surface>(
      this->add_resource(fcppt::make_unique_ptr<sge::d3d9::surface::depth_stencil_offscreen>(
          *device_,
          _parameters,
          present_parameters_.MultiSampleType,
          sge::d3d9::multi_sample_quality(present_parameters_.MultiSampleQuality))));
}

sge::renderer::texture::volume_unique_ptr
sge::d3d9::device::create_volume_texture(sge::renderer::texture::volume_parameters const &_param)
{
  return fcppt::unique_ptr_to_base<sge::renderer::texture::volume>(
      this->add_resource(fcppt::make_unique_ptr<sge::d3d9::texture::volume>(*device_, _param)));
}

sge::renderer::texture::cube_unique_ptr
sge::d3d9::device::create_cube_texture(sge::renderer::texture::cube_parameters const &_param)
{
  return fcppt::unique_ptr_to_base<sge::renderer::texture::cube>(
      this->add_resource(fcppt::make_unique_ptr<sge::d3d9::texture::cube>(*device_, _param)));
}

sge::renderer::vertex::declaration_unique_ptr sge::d3d9::device::create_vertex_declaration(
    sge::renderer::vertex::declaration_parameters const &_parameters)
{
  return fcppt::unique_ptr_to_base<sge::renderer::vertex::declaration>(
      fcppt::make_unique_ptr<sge::d3d9::vertex::declaration>(*device_, _parameters));
}

sge::renderer::vertex::buffer_unique_ptr
sge::d3d9::device::create_vertex_buffer(sge::renderer::vertex::buffer_parameters const &_parameters)
{
  return fcppt::unique_ptr_to_base<sge::renderer::vertex::buffer>(
      this->add_resource(fcppt::make_unique_ptr<sge::d3d9::vertex::buffer>(
          *device_,
          fcppt::cast::static_downcast<sge::d3d9::vertex::declaration const &>(
              _parameters.declaration())
              .format()
              .parts()
              .at(_parameters.part().get()),
          _parameters)));
}

sge::renderer::index::buffer_unique_ptr
sge::d3d9::device::create_index_buffer(sge::renderer::index::buffer_parameters const &_parameters)
{
  return fcppt::unique_ptr_to_base<sge::renderer::index::buffer>(
      this->add_resource(fcppt::make_unique_ptr<sge::d3d9::index_buffer>(*device_, _parameters)));
}

sge::renderer::occlusion_query::object_unique_ptr sge::d3d9::device::create_occlusion_query()
{
  return sge::d3d9::occlusion_query::create(*device_);
}

sge::renderer::state::core::blend::object_unique_ptr sge::d3d9::device::create_blend_state(
    sge::renderer::state::core::blend::parameters const &_parameters)
{
  return sge::d3d9::state::core::blend::create(*device_, _parameters);
}
...
{
  return sge::d3d9::state::core::depth_stencil::create(*device_, _parameters);
}
...
{
  return sge::d3d9::state::core::rasterizer::create(*device_, _parameters);
}

sge::renderer::state::core::sampler::object_unique_ptr sge::d3d9::device::create_sampler_state(
    sge::renderer::state::core::sampler::parameters const &_parameters)
{
  return sge::d3d9::state::core::sampler::create(*device_, _parameters);
}

#if defined(SGE_RENDERER_HAVE_CG)
sge::cg::profile::object
sge::d3d9::device::create_cg_profile(sge::cg::profile::shader_type const _shader_type)
{
  return sge::d3d9::cg::profile::create(*device_, _shader_type);
}

sge::cg::program::compile_options sge::d3d9::device::cg_compile_options(
    sge::cg::context::object const &, sge::cg::profile::object const &_profile)
{
  return sge::d3d9::cg::program::optimal_options(*device_, _profile);
}

sge::renderer::cg::loaded_program_unique_ptr
sge::d3d9::device::load_cg_program(sge::cg::program::object_ref const _program)
{
  return sge::d3d9::cg::program::create_loaded(*device_, _program.get());
}

sge::renderer::cg::loaded_texture_unique_ptr sge::d3d9::device::load_cg_texture(
    sge::cg::parameter::object const &_parameter, sge::renderer::texture::base_ref const _texture)
{
  return sge::d3d9::cg::texture::create_loaded(
      *device_, _parameter, _texture.get(), caps_.texture_stages());
}

sge::cg::program::source sge::d3d9::device::transform_cg_vertex_program(
    sge::renderer::vertex::declaration const &_vertex_declaration,
    sge::cg::program::source const &_source)
{
  return sge::d3d9::cg::program::transform_vertex(_vertex_declaration, _source);
}
#endif

sge::renderer::target::onscreen &sge::d3d9::device::onscreen_target() const
{
  return *onscreen_target_;
}

sge::renderer::caps::device const &sge::d3d9::device::caps() const { return caps_; }

sge::renderer::display_mode::optional_object sge::d3d9::device::display_mode() const
{
  return sge::renderer::display_mode::optional_object(sge::d3d9::current_display_mode(*device_));
}

void sge::d3d9::device::fullscreen(
    sge::renderer::display_mode::optional_fullscreen const &_fullscreen)
{
  // TODO
}

sge::renderer::display_mode::container sge::d3d9::device::display_modes() const
{
  return display_modes_;
}

sge::renderer::context::ffp_unique_ptr
sge::d3d9::device::begin_rendering_ffp(sge::renderer::target::base_ref const _target)
{
  sge::d3d9::devicefuncs::begin_scene(*device_);

  return sge::d3d9::render_context::create(sge::d3d9::render_context::parameters(
      *device_,
      _target.get(),
...
void sge::d3d9::device::end_rendering_ffp(sge::renderer::context::ffp &_context)
{
  this->end_rendering(_context);
}

sge::renderer::state::ffp::alpha_test::object_unique_ptr sge::d3d9::device::create_alpha_test_state(
    sge::renderer::state::ffp::alpha_test::parameters const &_parameters)
{
  return sge::d3d9::state::ffp::alpha_test::create(*device_, _parameters);
}

sge::renderer::state::ffp::clip_plane::object_unique_ptr sge::d3d9::device::create_clip_plane_state(
    sge::renderer::state::ffp::clip_plane::parameters const &_parameters)
{
  return sge::d3d9::state::ffp::clip_plane::create(*device_, _parameters);
}

sge::renderer::state::ffp::fog::object_unique_ptr
sge::d3d9::device::create_fog_state(sge::renderer::state::ffp::fog::parameters const &_parameters)
{
  return sge::d3d9::state::ffp::fog::create(*device_, _parameters);
}

sge::renderer::state::ffp::lighting::object_unique_ptr sge::d3d9::device::create_lighting_state(
    sge::renderer::state::ffp::lighting::parameters const &_parameters)
{
  return sge::d3d9::state::ffp::lighting::create(*device_, _parameters);
}

sge::renderer::state::ffp::lighting::light::object_unique_ptr sge::d3d9::device::create_light_state(
    sge::renderer::state::ffp::lighting::light::parameters const &_parameters)
{
  return sge::d3d9::state::ffp::lighting::light::create(*device_, _parameters);
}
...
{
  return sge::d3d9::state::ffp::lighting::material::create(*device_, _parameters);
}

sge::renderer::state::ffp::misc::object_unique_ptr
sge::d3d9::device::create_misc_state(sge::renderer::state::ffp::misc::parameters const &_parameters)
{
  return sge::d3d9::state::ffp::misc::create(*device_, _parameters);
}

sge::renderer::state::ffp::sampler::object_unique_ptr sge::d3d9::device::create_ffp_sampler_state(
    sge::renderer::state::ffp::sampler::parameters const &_parameters)
{
  return sge::d3d9::state::ffp::sampler::create(*device_, _parameters);
}

sge::renderer::state::ffp::transform::object_unique_ptr sge::d3d9::device::create_transform_state(
    sge::renderer::state::ffp::transform::parameters const &_parameters)
{
  return sge::d3d9::state::ffp::transform::create(*device_, _parameters);
}

template <typename Ptr>
fcppt::unique_ptr<Ptr> sge::d3d9::device::add_resource(fcppt::unique_ptr<Ptr> &&_ptr)
{
  resources_.add(*_ptr);
...
void sge::d3d9::device::reinit() { resources_.reinit(); }

void sge::d3d9::device::release() { resources_.release(); }

void sge::d3d9::device::reset()
{
  this->release();

  while (device_->TestCooperativeLevel() == D3DERR_DEVICELOST ||
         !sge::d3d9::devicefuncs::reset(*device_, present_parameters_))
    std::this_thread::sleep_for(std::chrono::milliseconds(1));

  this->reinit();
}

awl::event::container sge::d3d9::device::on_window_event(awl::window::event::base const &_event)
{
  fcppt::optional::maybe_void(
      ...
      { this->on_resize(_resize.get()); });

  return awl::event::container{};
}

awl::event::container sge::d3d9::device::on_system_event(awl::event::base const &_event)
{
  return fcppt::optional::to_container<awl::event::container>(fcppt::optional::make_if(
      awl::timer::match(_event, *draw_timer_),
      [this]
      {
        return fcppt::unique_ptr_to_base<awl::event::base>(
            fcppt::make_unique_ptr<sge::renderer::event::render>(
                fcppt::make_ref(this->window_.awl_object())));
      }));
}

void sge::d3d9::device::on_resize(awl::window::event::resize const &_resize)
{
  if (present_parameters_.Windowed == FALSE)
    return;

  if (fcppt::math::dim::contents(_resize.dim()) == 0u)
    return;

  present_parameters_.BackBufferWidth = _resize.dim().w();

  present_parameters_.BackBufferHeight = _resize.dim().h();
