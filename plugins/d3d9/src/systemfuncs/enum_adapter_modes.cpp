    ...
    UINT const _mode)
{
  ...
  switch (_d3d9.EnumAdapterModes(_adapter.get(), _format, _mode, &result))
  {
  case D3D_OK:
    return sge::d3d9::optional_display_mode(result);
  case D3DERR_NOTAVAILABLE:
    return sge::d3d9::optional_display_mode();
  default:
    throw sge::renderer::exception(FCPPT_TEXT("EnumAdapterModes failed"));
  }
}
