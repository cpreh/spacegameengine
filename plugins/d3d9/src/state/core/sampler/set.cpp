...
void sge::d3d9::state::core::sampler::set(
...
{
  for (auto const &sampler : _states)
    fcppt::optional::maybe(
        sampler.second,
        [&_default]() { return fcppt::make_cref(_default); },
        [](fcppt::reference<sge::renderer::state::core::sampler::object const> const _ref)
        {
          return fcppt::make_cref(
              fcppt::cast::static_downcast<sge::d3d9::state::core::sampler::object const &>(
                  _ref.get()));
        })
        .get()
        .set(sampler.first);
