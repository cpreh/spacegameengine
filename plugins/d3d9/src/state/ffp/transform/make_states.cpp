...
sge::d3d9::state::ffp::transform::state const sge::d3d9::state::ffp::transform::make_states(
    sge::renderer::state::ffp::transform::parameters const &_parameters)
{
  return sge::d3d9::state::ffp::transform::state(
      sge::d3d9::convert::matrix(fcppt::math::matrix::transpose(_parameters.matrix())));
}
