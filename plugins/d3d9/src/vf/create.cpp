...
sge::d3d9::vf::element_vector sge::d3d9::vf::create(
    ...
    sge::d3d9::vf::texture_coordinate_count const _texture_coordinates)
{
  ...
  for (sge::renderer::vf::dynamic::part const &part : _format.parts())
  {
    for (sge::renderer::vf::dynamic::ordered_element const &ordered_element : part.elements())
    {
      D3DDECLUSAGE const usage(sge::d3d9::vf::convert::role(ordered_element.element()));

      D3DVERTEXELEMENT9 const new_elem = {
          stream,
          fcppt::cast::size<WORD>(ordered_element.offset().get()),
          fcppt::cast::size<BYTE>(sge::d3d9::vf::convert::type(ordered_element.element())),
          D3DDECLMETHOD_DEFAULT,
          fcppt::cast::size<BYTE>(usage),
          sge::d3d9::vf::convert::index(ordered_element.element(), _texture_coordinates)};

      vertex_elements.push_back(new_elem);
    }
