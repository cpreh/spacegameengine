...
sge::d3d9::surface::d3d_unique_ptr sge::d3d9::devicefuncs::create_depth_stencil_surface(
    ...
    sge::d3d9::multi_sample_quality const _multi_sample_quality)
{
  ...
  if (_device.CreateDepthStencilSurface(
          static_cast<UINT>(_parameters.size().w()),
          static_cast<UINT>(_parameters.size().h()),
          sge::d3d9::convert::depth_stencil_format(_parameters.format()),
          _samples,
          _multi_sample_quality.get(),
          TRUE, // discard
          &ret,
          0) != D3D_OK)
    throw sge::renderer::exception(FCPPT_TEXT("CreateDepthStencilSurface() failed!"));
