...
void sge::d3d9::devicefuncs::draw_primitive(
...
{
  if (_device.DrawPrimitive(
          sge::d3d9::convert::primitive_type(_primitive_type),
          static_cast<UINT>(_first_vertex.get()),
          static_cast<UINT>(
              sge::renderer::vertex::to_primitive_count(_num_vertices, _primitive_type).get())) !=
      D3D_OK)
    throw sge::renderer::exception(FCPPT_TEXT("DrawPrimitive() failed!"));
}
