...
sge::d3d9::index_buffer::index_buffer(
    ...
    : sge::renderer::index::buffer(),
      ...
      stride_(sge::renderer::index::dynamic::format_stride(format_)),
      lock_dest_(nullptr)
{
  this->init();
}
...
sge::renderer::index::dynamic::view sge::d3d9::index_buffer::lock(
    sge::renderer::lock_segment const &_segment, sge::renderer::lock_mode const _mode)
{
  return this->do_lock<sge::renderer::index::dynamic::view>(
      _segment, sge::renderer::lock_flags::from_mode(_mode));
}

sge::renderer::index::dynamic::const_view
sge::d3d9::index_buffer::lock_c(sge::renderer::lock_segment const &_segment) const
{
...
void sge::d3d9::index_buffer::unlock() const
{
  ...
  if (this->get().Unlock() != D3D_OK)
    throw sge::renderer::exception(FCPPT_TEXT("Cannot unlock index buffer!"));

  lock_dest_ = nullptr;
}

sge::renderer::dim1 sge::d3d9::index_buffer::size() const
{
  return sge::renderer::dim1{size_.get()};
}

sge::renderer::resource_flags_field sge::d3d9::index_buffer::resource_flags() const
{
  return resource_flags_;
}

sge::renderer::index::dynamic::format sge::d3d9::index_buffer::format() const { return format_; }
...
void sge::d3d9::index_buffer::init()
{
  ...
  if (device_.CreateIndexBuffer(
          static_cast<UINT>(size_.get() * stride_),
          sge::d3d9::convert::resource_flags(this->resource_flags()).get(),
          sge::d3d9::convert::index_format(this->format()),
          this->pool(),
          &ret,
          nullptr) != D3D_OK)
    throw sge::renderer::exception(FCPPT_TEXT("CreateIndexBuffer() failed!"));

  buffer_ = sge::d3d9::index_buffer::optional_d3d_index_buffer_unique_ptr(
      sge::d3d9::index_buffer::d3d_index_buffer_unique_ptr(ret));
}
...
void sge::d3d9::index_buffer::on_reset() { this->init(); }

template <typename View>
View sge::d3d9::index_buffer::do_lock(
    ...
    sge::renderer::lock_flags::method const _method) const
{
  ...
  if (this->get().Lock(
          fcppt::cast::size<UINT>(_segment.pos().x() * stride_),
          fcppt::cast::size<UINT>(_segment.size().w() * stride_),
          &dest,
          sge::d3d9::convert::lock_flags(_method, this->resource_flags()).get()) != D3D_OK)
    throw sge::renderer::exception(FCPPT_TEXT("Cannot lock index buffer!"));

  lock_dest_ = fcppt::cast::from_void_ptr<sge::renderer::raw_pointer>(dest);

  return View(lock_dest_, _segment.size().w(), format_);
}
