...
sge::dinput::joypad::ff::scoped_download::scoped_download(IDirectInputEffect &_effect)
    : effect_{_effect}
{
  switch (effect_.Download())
  {
  case DI_OK:
    return;
  case static_cast<HRESULT>(DIERR_DEVICEFULL):
    throw sge::input::exception{FCPPT_TEXT("FF device is full!")};
  default:
    throw sge::input::exception{FCPPT_TEXT("Failed downloading an effect!")};
  }
}
