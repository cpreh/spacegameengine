    ...
    sge::libpng::file_rep const &_rep)
{
  ...
  sge::libpng::write_ptr const write_ptr(fcppt::make_ref(error_context));
  ...
  ::png_set_IHDR(
      write_ptr.ptr(),
      info.get(),
      fcppt::cast::size<png_uint_32>(_rep.size().w()),
      fcppt::cast::size<png_uint_32>(_rep.size().h()),
      sge::libpng::bit_depth_from_format(_rep.format()),
      sge::libpng::color_type_from_format(_rep.format()),
      PNG_INTERLACE_NONE,
      PNG_COMPRESSION_TYPE_DEFAULT,
      PNG_FILTER_TYPE_DEFAULT);

  ::png_write_info(write_ptr.ptr(), info.get());

  sge::libpng::bytes_per_pixel const bytes_per_pixel(
      sge::image::color::element_count(sge::libpng::to_sge_format(_rep.format())) *
  ...
  ::png_write_image(write_ptr.ptr(), row_ptrs.data());

  ::png_write_end(write_ptr.ptr(), info.get());
}
