...
sge::libpng::file_rep_from_view(sge::image2d::view::const_object const &_view)
{
  return fcppt::optional::map(
      sge::libpng::from_sge_format(sge::image2d::view::format(_view)),
      [&_view](sge::libpng::format const _png_dest_format)
      {
        sge::image2d::dim const size(sge::image2d::view::size(_view));

        sge::image::color::format const dest_format(sge::libpng::to_sge_format(_png_dest_format));
        ...
        sge::libpng::byte_vector bytes{fcppt::container::buffer::to_raw_vector(
            fcppt::container::buffer::read_from<
                ...
                fcppt::math::dim::contents(size) * sge::image::color::format_stride(dest_format),
                [size, dest_format, &_view](
                    ...
                    sge::libpng::byte_vector::size_type const _byte_size)
                {
                  sge::image2d::algorithm::copy_and_convert(
                      ...
                      sge::image2d::view::make(
                          _data, size, dest_format, fcppt::math::dim::null<sge::image2d::pitch>()),
        ...
        return sge::libpng::file_rep(size, _png_dest_format, std::move(bytes));
      });
}
