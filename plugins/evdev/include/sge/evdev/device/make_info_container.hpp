...
template <typename Id, typename Info, typename Enum, typename Function>
sge::evdev::device::basic_info<Id, Info> make_info_container(
    sge::evdev::device::read_bits_result<Enum> const &_bits, Function const &_function)
{
    ...
    if (_bits[index])
    {
      event_map.insert(std::make_pair(
          sge::evdev::device::event_type{
              fcppt::cast::enum_to_int<sge::evdev::device::event_type_value>(index)},
          fcppt::strong_typedef_construct_cast<typename basic_info::id, fcppt::cast::size_fun>(
              ids.size())));

      ids.push_back(_function(index));
    }
  }

  return basic_info(info_container(std::move(ids)), std::move(event_map));
}
