...
public:
  ...
  [[nodiscard]] awl::event::container on_event();

  [[nodiscard]] awl::backends::posix::fd posix_fd();

  [[nodiscard]] std::filesystem::path const &path() const;

protected:
  [[nodiscard]] sge::evdev::device::fd &fd();

private:
  [[nodiscard]] virtual awl::event::optional_base_unique_ptr
  process_event(sge::evdev::device::event const &) = 0;
