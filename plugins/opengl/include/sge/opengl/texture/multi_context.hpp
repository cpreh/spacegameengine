...
class multi_context : public sge::opengl::context::base
{
  FCPPT_NONMOVABLE(multi_context);

public:
  using parameter = sge::opengl::info::context const &;

  explicit multi_context(sge::opengl::info::context const &);

  ~multi_context() override;

  [[nodiscard]] sge::opengl::texture::optional_multi_config const &config() const;
