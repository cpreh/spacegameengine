...
sge::opengl::texture::basic_lockable_buffer<Types>::basic_lockable_buffer(
    format_type const _format,
    const_config_ref const _config,
    sge::opengl::texture::basic_buffer_parameters const &_parameters)
    : gl_base(_format, _config, _parameters),
      log_{_parameters.log()},
      context_(_parameters.context()),
      config_(_config),
      type_(_parameters.type()),
      resource_flags_(_parameters.resource_flags()),
      color_order_(_parameters.color_order()),
      color_base_type_(_parameters.color_base_type()),
      internal_color_format_(_parameters.internal_color_format()),
      stride_(sge::image::traits::pixel::format_stride<color_tag>(this->format())),
      lock_(),
      lock_area_()
{
}
...
{
template <typename Types>
basic_lockable_buffer<Types>::~basic_lockable_buffer() = default;
}
...
{
  this->lock_me(_area, sge::renderer::lock_flags::from_mode(_mode));
...
void sge::opengl::texture::basic_lockable_buffer<Types>::unlock() const
{
  ...
  if (sge::renderer::lock_flags::write(cur_lock->method()))
  {
    // If this is also a read lock
    // we must copy the current view, which is
    // a slice into the whole texture retrieved,
    // to the destination buffer.
    if (sge::renderer::lock_flags::read(cur_lock->method()))
    {
      // this will only copy, not convert!
      sge::image::algorithm::copy<image_tag>(
          this->lock_view(),
          sge::image::view::make<image_tag>(
              cur_lock->write_view_pointer(),
              this->lock_dim(),
              this->format(),
              fcppt::math::dim::null<pitch>()),
    ...
    sge::opengl::texture::scoped_work_binding const binding(
        log_.get(), context_.get(), type_, this->id());

    sge::opengl::set_unpack_alignment(
        context_.get(), sge::opengl::stride_to_unpack_alignment(stride_));
    ...
    Types::dim_types::sub_function().get()(
        ...
        fcppt::optional::from(
            ...
            [this] {
              return lock_area(
                  fcppt::math::vector::null<typename lock_area::vector>(), this->size());
            }),
        cur_lock->write_pointer());
  }

  lock_ = optional_lock_unique_ptr();
}

template <typename Types>
void sge::opengl::texture::basic_lockable_buffer<Types>::lock_me(
    lock_area const &_lock_area, sge::renderer::lock_flags::method const _method) const
{
  if (!sge::opengl::range_check(this->size(), _lock_area))
  {
    throw sge::renderer::exception(
        (fcppt::format(FCPPT_TEXT("ogl: lock (%1%) out of range! dim is %2%!")) % _lock_area %
         this->size())
            .str());
  }
  ...
  lock_unique_ptr const &cur_lock{fcppt::optional::assign(
      ...
      sge::opengl::texture::create_lock(
          ...
          sge::opengl::texture::read_size{
              sge::opengl::buffer::count{fcppt::math::dim::contents(this->size())}},
          sge::opengl::texture::write_size{
              sge::opengl::buffer::count{fcppt::math::dim::contents(_lock_area.size())}},
  ...
  lock_area_ =
      ...
      _lock_area.size() == this->size() ? optional_lock_area() : optional_lock_area(_lock_area);
}
...
sge::opengl::texture::basic_lockable_buffer<Types>::lock_view()
{
  ...
  bool const reading(sge::renderer::lock_flags::read(cur_lock->method()));

  view const ret(sge::image::view::make<image_tag>(
      reading ? cur_lock->read_view_pointer() : cur_lock->write_view_pointer(),
      reading ? this->size() : this->lock_dim(),
      this->format(),
      fcppt::math::dim::null<pitch>()));

  return reading ? fcppt::optional::maybe(
                       lock_area_,
                       fcppt::const_(ret),
                       [&ret](lock_area const &_area) {
                         return sge::image::view::flipped<image_tag>(
                             sge::image::view::sub<image_tag>(ret, _area));
                       })
                 : ret;
}
...
sge::opengl::texture::basic_lockable_buffer<Types>::lock_view() const
{
  return sge::image::view::to_const<image_tag>(
      ...
      const_cast<sge::opengl::texture::basic_lockable_buffer<Types> *>(this)->lock_view());
}
...
sge::opengl::texture::basic_lockable_buffer<Types>::lock_dim() const
{
  return fcppt::optional::maybe(
      ...
      [](lock_area const &_area) { return _area.size(); });
}
