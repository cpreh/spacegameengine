...
{
public:
  using dim = sge::renderer::basic_dim<Dim>;

  using config_type = sge::opengl::texture::config<Dim>;

  using const_config_ref = fcppt::reference<config_type const>;

  using init_function_type = sge::opengl::texture::init_function<Dim>;
  ...
  [[nodiscard]] sge::opengl::color_order format_order() const;

  [[nodiscard]] sge::opengl::color_base_type format_base_type() const;

  [[nodiscard]] sge::opengl::internal_color_format internal_format() const;

  [[nodiscard]] dim const &size() const;

  [[nodiscard]] init_function_type init_function() const;
...
#define SGE_OPENGL_TEXTURE_MIPMAP_DECLARE_PARAMETERS(seq, _, dimension) \
  extern template class sge::opengl::texture::mipmap::parameters<dimension>;

BOOST_PP_SEQ_FOR_EACH(SGE_OPENGL_TEXTURE_MIPMAP_DECLARE_PARAMETERS, _, SGE_OPENGL_TEXTURE_PP_DIMS)
