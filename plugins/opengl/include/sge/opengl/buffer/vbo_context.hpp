...
class vbo_context : public sge::opengl::context::base
{
  FCPPT_NONMOVABLE(vbo_context);

public:
  using parameter = sge::opengl::context::object_ref;

  explicit vbo_context(parameter);

  ~vbo_context() override;

  [[nodiscard]] sge::opengl::buffer::base &index_buffer() const;

  [[nodiscard]] sge::opengl::buffer::base &vertex_buffer() const;
