...
sge::opengl::state::actor_vector sge::opengl::state::ffp::lighting::material::make_actors(
    sge::renderer::state::ffp::lighting::material::parameters const &_parameters)
{
  ...
  return sge::opengl::state::actor_vector{
      sge::opengl::state::ffp::lighting::material::color(
          face, GL_AMBIENT, _parameters.ambient().get()),
      sge::opengl::state::ffp::lighting::material::color(
          face, GL_DIFFUSE, _parameters.diffuse().get()),
      sge::opengl::state::ffp::lighting::material::color(
          face, GL_SPECULAR, _parameters.specular().get()),
      sge::opengl::state::ffp::lighting::material::color(
          face, GL_EMISSION, _parameters.emissive().get()),
      sge::opengl::state::ffp::lighting::material::float_(
          face, GL_SHININESS, fcppt::cast::size<GLfloat>(_parameters.shininess().get()))};
}
