...
void sge::opengl::state::core::sampler::set(
    ...
    sge::renderer::state::core::sampler::const_optional_object_ref_map const &_objects)
{
  ...
  for (auto const &sampler : _objects)
  {
    sge::renderer::texture::stage const stage(sampler.first);

    fcppt::optional::maybe_void(
        sge::opengl::texture::get_stage_type(_context, stage),
        [&_log, &_context, stage](sge::opengl::texture::type const _bound_type)
        {
          sge::opengl::texture::active_level const active_level(_log, _context, stage);

          sge::opengl::texture::render_binding const render_binding(active_level, _bound_type);

          sge::opengl::texture::set_samplers(render_binding, _context);
        });
  }
}
