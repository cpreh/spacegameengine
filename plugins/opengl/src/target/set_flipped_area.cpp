...
void sge::opengl::target::set_flipped_area(
    sge::opengl::target::area_function _function,
    sge::renderer::pixel_rect const &_area,
    sge::renderer::screen_unit const _height)
{
  _function(
      fcppt::cast::size<GLint>(_area.pos().x()),
      fcppt::cast::size<GLint>(
          fcppt::cast::to_signed(_height) - _area.size().h() - _area.pos().y()),
      fcppt::cast::size<GLsizei>(_area.size().w()),
      fcppt::cast::size<GLsizei>(_area.size().h()));
