...
void sge::opengl::windows::change_display_settings(
    fcppt::log::object &_log, sge::renderer::display_mode::object const &_mode)
{
  DEVMODE settings;
  ...
  fcppt::optional::maybe_void(
      _mode.refresh_rate(),
      [&settings](sge::renderer::display_mode::refresh_rate const _refresh_rate)
      {
  ...
  if (::ChangeDisplaySettings(&settings, CDS_FULLSCREEN) != DISP_CHANGE_SUCCESSFUL)
    FCPPT_LOG_ERROR(
        _log,
        fcppt::log::out << FCPPT_TEXT("Cannot change resolution to ") << _mode
