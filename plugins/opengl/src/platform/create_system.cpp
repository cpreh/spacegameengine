...
using either_type = fcppt::either::object<fcppt::string, sge::opengl::platform::system_unique_ptr>;
        ...
        return fcppt::either::from_optional(
            fcppt::optional::map(
                ...
                [&_create](fcppt::reference<Arg> const _system) {
                  return fcppt::unique_ptr_to_base<sge::opengl::platform::system>(_create(_system));
                }),
            []
            {
              return fcppt::string{
                  FCPPT_TEXT("Failed to convert to \"") +
                  fcppt::from_std_string(fcppt::type_name_from_info(typeid(Arg))) +
                  FCPPT_TEXT('"')};
            });
      }};
}
  ...
  return fcppt::either::to_exception(
      fcppt::either::first_success(fcppt::array::make(
          ...
          function_type{[_log]
                        {
                          return either_type{
                              fcppt::unique_ptr_to_base<sge::opengl::platform::system>(
                                  fcppt::make_unique_ptr<sge::opengl::windows::system>(_log))};
                        }},
#endif
