...
sge::opengl::xrandr::output_info::output_info(
    ...
    : info_(::XRRGetOutputInfo(_display.get().get(), _resources.get().get(), _output))
{
  if (info_ == nullptr)
  {
    throw sge::renderer::exception{FCPPT_TEXT("XRRGetOutputInfo")};
  }
}

sge::opengl::xrandr::output_info::~output_info() { ::XRRFreeOutputInfo(info_); }

sge::renderer::display_mode::dimensions sge::opengl::xrandr::output_info::dimensions() const
{
  return sge::renderer::display_mode::dimensions(sge::renderer::screen_size(
      fcppt::cast::size<sge::renderer::screen_unit>(info_->mm_width),
      fcppt::cast::size<sge::renderer::screen_unit>(info_->mm_height)));
}
