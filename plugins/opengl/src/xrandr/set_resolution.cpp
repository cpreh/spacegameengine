    ...
    sge::opengl::xrandr::optional_refresh_rate const &_refresh_rate)
{
  fcppt::optional::maybe(
      _refresh_rate,
      [&_window, &_config, _mode_index, _rotation]
      {
        if (::XRRSetScreenConfig(
                ...
                CurrentTime) != 0)
        {
          throw sge::renderer::exception{FCPPT_TEXT("Cannot change screen mode!")};
        }
      },
      [&_window, &_config, _mode_index, _rotation](sge::opengl::xrandr::refresh_rate const _rate)
      {
        if (::XRRSetScreenConfigAndRate(
                _window.display().get().get(),
                _config.get(),
                _window.get(),
                _mode_index.get(),
                ...
                CurrentTime) != 0)
        {
          throw sge::renderer::exception{FCPPT_TEXT("Cannot change screen mode with rate!")};
        }
      });
}
