...
sge::opengl::vf::texpos_actor::texpos_actor(
    ...
    : sge::opengl::vf::pointer_actor(_param),
      log_{_log},
      context_(_param.context()),
      elements_(
          fcppt::cast::size<GLint>(fcppt::cast::to_signed(_element.type().element_count().get()))),
      format_(sge::opengl::vf::convert_element_type(_element.type().element_type())),
      index_(_element.index().get())
{
  fcppt::optional::maybe(
      sge::opengl::context::use<sge::opengl::texture::multi_context>(
          context_, context_.get().info())
          .config(),
      [this]
      {
        if (index_.get() != 0U)
      ...
      [this](sge::opengl::texture::multi_config const &_config)
      {
        if (index_.get() >= _config.max_level().get())
        {
          throw sge::renderer::exception{
              (fcppt::format(FCPPT_TEXT("opengl texture coordinates exceeded: Allowed are %1%.")) %
               _config.max_level())
                  .str()};
        }
      });
...
sge::opengl::vf::texpos_actor::~texpos_actor() = default;
...
{
  _combiner.get().enable_texture(index_);

  sge::opengl::texture::funcs::set_client_level(log_.get(), context_.get(), index_);
  ...
  SGE_OPENGL_CHECK_STATE(FCPPT_TEXT("glTexCoordPointer failed"), sge::renderer::exception)
}

void sge::opengl::vf::texpos_actor::unuse(
    sge::opengl::vf::client_state_combiner_ref const _combiner) const
{
  _combiner.get().disable_texture(index_);
}
