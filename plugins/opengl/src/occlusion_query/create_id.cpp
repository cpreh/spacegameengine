...
sge::opengl::occlusion_query::create_id(sge::opengl::occlusion_query::config const &_config)
{
  GLuint id{};

  sge::opengl::call_fun_ref(_config.gen_queries(), 1, &id);

  SGE_OPENGL_CHECK_STATE(FCPPT_TEXT("Generating a query failed"), sge::renderer::exception)

  return sge::opengl::occlusion_query::id(id);
}
