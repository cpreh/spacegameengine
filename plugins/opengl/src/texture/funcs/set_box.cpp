    ...
    sge::renderer::const_raw_pointer const _src)
{
  ...
  if (!sge::opengl::range_check(_dim, _lock_box))
  {
    throw sge::renderer::exception{
        (fcppt::format(
  ...
  sge::renderer::lock_box::dim const lock_size(_lock_box.size());

  sge::opengl::call_fun_ref(
      _config.tex_sub_image_3d(),
      _buffer_type.get(),
      fcppt::cast::to_signed(_level.get()),
      fcppt::cast::size<GLint>(fcppt::cast::to_signed(_lock_box.left())),
      fcppt::cast::size<GLint>(fcppt::cast::to_signed(_lock_box.top())),
      fcppt::cast::size<GLint>(fcppt::cast::to_signed(_lock_box.front())),
      fcppt::cast::size<GLsizei>(fcppt::cast::to_signed(lock_size.w())),
      fcppt::cast::size<GLsizei>(fcppt::cast::to_signed(lock_size.h())),
      fcppt::cast::size<GLsizei>(fcppt::cast::to_signed(lock_size.d())),
      _format.get(),
      _format_type.get(),
      fcppt::cast::to_void_ptr(_src));

  SGE_OPENGL_CHECK_STATE(
      (fcppt::format(FCPPT_TEXT("glTexSubImage3D with (box=%1%, dim=%2%) failed")) % _lock_box %
       _dim)
          .str(),
