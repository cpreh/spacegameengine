...
{
  FCPPT_NONMOVABLE(screen_resources);

public:
  explicit screen_resources(awl::backends::x11::window::const_base_ref);

  ~screen_resources();

  [[nodiscard]] XRRScreenResources *get() const;

  [[nodiscard]] XRRScreenResources const &get_ref() const;
