...
template <sge::renderer::vf::vertex_size Size>
using static_size = std::integral_constant<sge::renderer::vf::vertex_size, Size>;
...
TEST_CASE("renderer/vf/pos color interleaved", "[sge]")
{
  ...
  using color_format = sge::image::color::bgra8_format;

  using color_type = sge::renderer::vf::color<color_format>;

  using format_part = sge::renderer::vf::part<pos3_type, color_type>;
  ...
  CHECK(
      ...
      sge::renderer::vf::dynamic::part_list{sge::renderer::vf::dynamic::part{
          sge::renderer::vf::dynamic::ordered_element_list{
              sge::renderer::vf::dynamic::ordered_element{
                  sge::renderer::vf::dynamic::element{sge::renderer::vf::dynamic::any{
                      sge::renderer::vf::dynamic::pos{sge::renderer::vf::dynamic::vector{
                          sge::renderer::vf::dynamic::element_type::float_,
                          sge::renderer::vf::dynamic::element_count{3U}}}}},
                  sge::renderer::vf::dynamic::offset{0U}},
              sge::renderer::vf::dynamic::ordered_element{
                  sge::renderer::vf::dynamic::element{sge::renderer::vf::dynamic::any{
                      sge::renderer::vf::dynamic::color{sge::image::color::format::bgra8}}},
                  sge::renderer::vf::dynamic::offset{3 * sizeof(float)}}},
          stride}});

  using view_type = sge::renderer::vf::view<format_part>;

  fcppt::array::object<sge::renderer::raw_value, 3 * format_part::stride::value> test_data{
      fcppt::no_init{}};
...

  {
    sge::renderer::vf::dynamic::view const dynamic_view{
        ...
        fcppt::make_cref(dynamic_format.parts().at(0)),
        sge::renderer::vf::dynamic::part_index{0U}};

    view_type const view{dynamic_view};

    view_type::iterator it{view.begin()};

    // Assignment of individual values
    (*it).set<sge::renderer::vf::labels::pos>(vec3(-1.F, 1.F, 0.F));

    (*it).set<sge::renderer::vf::labels::color>(
        sge::image::color::convert<color_format>(sge::image::color::predef::cyan()));
    ...
    *it = vertex{
        ...
        sge::renderer::vf::labels::color{} =
            sge::image::color::convert<color_format>(sge::image::color::predef::yellow())};

    ++it;

    // Assignment using the free function
    sge::renderer::vf::set_proxy(*it, sge::renderer::vf::labels::pos{}, vec3(1.F, 1.F, 0.F));

    sge::renderer::vf::set_proxy(
        *it,
        sge::renderer::vf::labels::color{},
        sge::image::color::convert<color_format>(sge::image::color::predef::magenta()));
  }
  ...
  auto const vec3_from(
      ...
      {
        vec3 result{fcppt::no_init()};

        std::memcpy(
            fcppt::cast::to_char_ptr<unsigned char *>(result.storage().data()),
            test_data.data() + // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                _index,
            3 * sizeof(float));
  ...
  // TODO(philipp)
  CHECK(vec3_from(0) == vec3{-1.F, 1.F, 0.F});

  CHECK(vec3_from(stride.get()) == vec3{-1.F, -1.F, 0.F});

  CHECK(vec3_from(2 * stride.get()) == vec3{1.F, 1.F, 0.F});
