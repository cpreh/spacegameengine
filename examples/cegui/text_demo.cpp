...
try
{
  fcppt::string const app_name(FCPPT_TEXT("simple gui test"));

  awl::main::scoped_output const output(
      fcppt::io::clog(), sge::config::own_log_path(sge::config::app_name(app_name)));
  ...
  sge::systems::instance<
      ...
      sys(sge::systems::make_list(sge::systems::window(sge::systems::window_source(
          sge::systems::original_window(sge::window::title(app_name)))))(
          ...
          sge::systems::config().log_settings(sge::systems::log_settings(sge::log::option_container{
              sge::log::option{sge::cegui::log_location(), fcppt::log::level::verbose}}))));
  ...
  sge::cegui::system gui_sys(
      ...
      sge::cegui::load_context(
          ...
          .font_directory(sge::config::media_path() / FCPPT_TEXT("fonts")),
  ...
  sge::cegui::syringe gui_syringe(sys.log_context(), fcppt::make_ref(gui_sys));

  sge::cegui::default_cursor gui_cursor{fcppt::make_ref(gui_syringe)};

  sge::cegui::default_focus gui_focus{fcppt::make_ref(gui_syringe)};
  ...
  sge::cegui::toolbox::scoped_gui_sheet const scoped_gui_sheet(
      fcppt::make_ref(gui_sys), fcppt::make_ref(scoped_layout.window()));

  auto const draw(
      [&frame_timer, &gui_sys, &sys]
      {
        gui_sys.update(
            std::chrono::duration_cast<sge::cegui::duration>(sge::timer::elapsed(frame_timer)));
        ...
        gui_sys.render(scoped_block.get());
      });

  return sge::window::loop(
      sys.window_system(),
      sge::window::loop_function{
          [&sys, &draw, &gui_cursor, &gui_focus](awl::event::base const &_event)
          {
            ...
            fcppt::optional::maybe_void(
                ...
                [&draw, &gui_cursor, &gui_focus](auto const &_variant)
                {
                  fcppt::variant::match(
                      _variant,
                      [&draw](fcppt::reference<sge::renderer::event::render const>) { draw(); },
                      [&gui_cursor, &gui_focus](
                          fcppt::reference<sge::input::event_base const> const _input_event)
                      {
                        gui_cursor.process_event(_input_event.get());

                        gui_focus.process_event(_input_event.get());
                      });
                });
          }});
}
