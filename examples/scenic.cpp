...
FCPPT_RECORD_MAKE_LABEL(record_to_file_label);

FCPPT_RECORD_MAKE_LABEL(track_from_file_label);

FCPPT_RECORD_MAKE_LABEL(recording_interval_label);

FCPPT_RECORD_MAKE_LABEL(ffp_label);

FCPPT_RECORD_MAKE_LABEL(scene_name_label);

using options_record = fcppt::record::object<
    fcppt::record::element<record_to_file_label, fcppt::optional::object<fcppt::string>>,
    fcppt::record::element<track_from_file_label, fcppt::optional::object<fcppt::string>>,
    fcppt::record::element<recording_interval_label, sge::camera::update_duration::rep>,
    fcppt::record::element<ffp_label, bool>,
    fcppt::record::element<scene_name_label, fcppt::string>>;

awl::main::exit_code main_program(options_record const &_options)
{
  if (fcppt::record::get<record_to_file_label>(_options).has_value() &&
      fcppt::record::get<track_from_file_label>(_options).has_value())
  {
    awl::show_error(FCPPT_TEXT(
        "You specified to record a file and use the tracking camera, that doesn't make sense.\n"));
  ...
  sge::systems::instance<
      ...
      sge::systems::window_source{sge::systems::original_window{
          sge::window::title{FCPPT_TEXT("sge scenic example")}}}})(sge::systems::renderer{
  ...
  fcppt::unique_ptr<sge::camera::base> const camera(fcppt::optional::maybe(
      fcppt::record::get<track_from_file_label>(_options),
      []
      {
        return fcppt::unique_ptr_to_base<
            sge::camera::base>(fcppt::make_unique_ptr<sge::camera::first_person::object>(
      ...
      {
        return fcppt::unique_ptr_to_base<sge::camera::base>(
            fcppt::make_unique_ptr<sge::camera::tracking::object>(
                ...
                sge::camera::tracking::json::keyframes_from_json(
                    sge::parse::json::parse_file_exn(std::filesystem::path(_track_from_file))
                        .array()),
                sge::camera::tracking::is_looping(true)));
      }));

  using exporter_unique_ptr = fcppt::unique_ptr<sge::camera::tracking::json::interval_exporter>;

  using optional_exporter_unique_ptr = fcppt::optional::object<exporter_unique_ptr>;

  optional_exporter_unique_ptr const exporter(fcppt::optional::map(
      fcppt::record::get<record_to_file_label>(_options),
      [&_options, &camera](fcppt::string const &_record_to_file)
      {
        return fcppt::make_unique_ptr<sge::camera::tracking::json::interval_exporter>(
            fcppt::make_cref(*camera),
            sge::camera::update_duration{fcppt::record::get<recording_interval_label>(_options)},
            std::filesystem::path(_record_to_file));
      }));

  sge::scenic::scene::manager scene_manager(
      fcppt::make_ref(sys.renderer_device_core()),
      fcppt::make_ref(sys.image_system()),
      sge::scenic::scene::prefer_cg_context(!fcppt::record::get<ffp_label>(_options)));

  sge::scenic::scene::object test_scene(
      sys.log_context(),
      fcppt::make_ref(scene_manager),
      fcppt::make_ref(sys.viewport_manager()),
      fcppt::make_ref(*camera),
      sge::scenic::scene::from_blender_file(
          sge::config::media_path() / FCPPT_TEXT("scenes") /
          fcppt::record::get<scene_name_label>(_options) / FCPPT_TEXT("description.json")));
  ...
  sge::scenic::grid::object simple_grid_xz{
      ...
      sge::scenic::grid::spacing{sge::scenic::grid::dim{1.0F, 1.0F}},
      sge::scenic::grid::distance_to_origin{0.0F},
  ...
  sge::graph::object graph{
      ...
      fcppt::make_ref(sys.renderer_device_ffp()),
      sge::graph::baseline{fcppt::literal<sge::graph::scalar>(
  ...
  auto const draw(
       ...
       &test_scene]
      {
        ...
        graph.push(fcppt::cast::size<sge::graph::scalar>(difference_since_last_frame.count()));

        fcppt::optional::maybe_void(
            exporter, [](exporter_unique_ptr const &_exporter) { _exporter->update(); });
...

        {
          ...
          test_scene.render(*wrapped_context);
        }

        simple_grid_xz.render(scoped_block.get(), sge::scenic::grid::depth_test{true});
...
try
{
  auto const argument_parser{fcppt::options::apply(
      fcppt::options::make_optional(fcppt::options::option<record_to_file_label, fcppt::string>{
          fcppt::options::optional_short_name{},
          fcppt::options::long_name{FCPPT_TEXT("record-to-file")},
          fcppt::options::no_default_value<fcppt::string>(),
          fcppt::options::optional_help_text{fcppt::options::help_text{
              FCPPT_TEXT("Use a first-person camera and record the keyframes to this file (use "
                         "recording-interval to control the keyframe interval)")}}}),
      fcppt::options::option<recording_interval_label, sge::camera::update_duration::rep>{
          fcppt::options::optional_short_name{},
          fcppt::options::long_name{FCPPT_TEXT("recording-interval")},
          fcppt::options::make_default_value(fcppt::optional::make(
              sge::camera::update_duration::rep{fcppt::literal<sge::renderer::scalar>(
                  0.25F // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
                  )})),
          fcppt::options::optional_help_text{fcppt::options::help_text{
              FCPPT_TEXT("Take a keyframe each recording-interval seconds (only makes sense "
                         "together with record-to-file)")}}},
      fcppt::options::make_optional(fcppt::options::option<track_from_file_label, fcppt::string>{
          fcppt::options::optional_short_name{},
          fcppt::options::long_name{FCPPT_TEXT("track-from-file")},
          fcppt::options::no_default_value<fcppt::string>(),
          fcppt::options::optional_help_text{fcppt::options::help_text{
              FCPPT_TEXT("Use a tracking camera and load keyframes from this file")}}}),
      fcppt::options::switch_<ffp_label>{
          fcppt::options::optional_short_name{},
          fcppt::options::long_name{FCPPT_TEXT("ffp")},
          fcppt::options::optional_help_text{fcppt::options::help_text{
              FCPPT_TEXT("Use an ffp context instead of a shader context")}}},
      fcppt::options::argument<scene_name_label, fcppt::string>{
          fcppt::options::long_name{FCPPT_TEXT("scene-name")},
          fcppt::options::optional_help_text{fcppt::options::help_text{
              FCPPT_TEXT("Scene name (denotes a path below media/scenes)")}}})};

  using result_type = fcppt::options::result_of<decltype(argument_parser)>;
