...
try
{
  sge::systems::instance<
      ...
      sys(sge::systems::make_list(
          sge::systems::window(sge::systems::window_source(sge::systems::original_window(
              sge::window::title(FCPPT_TEXT("sge texture filter example"))))))(
  ...
  auto const block_size(fcppt::math::dim::fill<sge::image2d::dim>(block_element_size));
  ...
  store_type const whole_store{
      ...
      store_type::init_function{
          [block_size](store_type::view_type const &_view)
          {
            store_type const white_store(
                block_size, sge::image::color::l8(sge::image::color::init::luminance() %= 1.));

            store_type const black_store(
                block_size, sge::image::color::l8(sge::image::color::init::luminance() %= 0.));

            for (sge::image::size_type const y : fcppt::make_int_range_count(num_blocks))
            {
              for (sge::image::size_type const x : fcppt::make_int_range_count(num_blocks))
              {
                sge::image2d::algorithm::copy(
                    sge::image2d::view::const_object(
                        (((x + y) % 2U) == 0U) ? white_store.wrapped_view()
                                               : black_store.wrapped_view()),
                    sge::image2d::view::sub(
                        sge::image2d::view::object(sge::image::view::wrap(_view)),
                        sge::image2d::rect(
                            sge::image2d::rect::vector(x, y) * block_size, block_size)),
  ...
  sge::font::string const anisotropy_string(
      SGE_FONT_LIT(' ') + fcppt::output_to_string<sge::font::string>(anisotropy) +
      SGE_FONT_LIT('x'));
  ...
  auto const make_sampler_parameters(
          ...
          -> sge::renderer::state::core::sampler::parameters
      {
        return sge::renderer::state::core::sampler::parameters(
            sge::renderer::state::core::sampler::address::default_(), _filter);
      });
  ...
  auto const make_anisotropic_sampler_parameters(
      [make_sampler_parameters](
          ...
          -> sge::renderer::state::core::sampler::parameters
      {
        return make_sampler_parameters(sge::renderer::state::core::sampler::filter::parameters(
  ...
  sge::renderer::state::core::sampler::object_unique_ptr const point_sampler(
      sys.renderer_device_core().create_sampler_state(
          make_sampler_parameters(sge::renderer::state::core::sampler::filter::point())));
  sge::renderer::state::core::sampler::object_unique_ptr const linear_sampler(
      sys.renderer_device_core().create_sampler_state(
          make_sampler_parameters(sge::renderer::state::core::sampler::filter::linear())));
  sge::renderer::state::core::sampler::object_unique_ptr const mipmap_sampler(
      sys.renderer_device_core().create_sampler_state(
          make_sampler_parameters(sge::renderer::state::core::sampler::filter::mipmap())));
  sge::renderer::state::core::sampler::object_unique_ptr const trilinear_sampler(
      sys.renderer_device_core().create_sampler_state(
          make_sampler_parameters(sge::renderer::state::core::sampler::filter::trilinear())));
  sge::renderer::state::core::sampler::object_unique_ptr const anisotropic_sampler(
      sys.renderer_device_core().create_sampler_state(make_anisotropic_sampler_parameters(
          sge::renderer::state::core::sampler::filter::anisotropic::mip::off, anisotropy)));
  sge::renderer::state::core::sampler::object_unique_ptr const anisotropic_mipmap_sampler(
      sys.renderer_device_core().create_sampler_state(make_anisotropic_sampler_parameters(
          sge::renderer::state::core::sampler::filter::anisotropic::mip::point, anisotropy)));
  sge::renderer::state::core::sampler::object_unique_ptr const anisotropic_trilinear_sampler(
      sys.renderer_device_core().create_sampler_state(make_anisotropic_sampler_parameters(
          sge::renderer::state::core::sampler::filter::anisotropic::mip::linear, anisotropy)));

  using string_filter_pair =
      std::pair<sge::font::string, sge::renderer::state::core::sampler::const_object_ref>;

  using filter_array = fcppt::array::object<
      string_filter_pair,
      7U // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
      >;

  filter_array const filters{
      std::make_pair(SGE_FONT_LIT("point"), fcppt::make_cref(*point_sampler)),
      std::make_pair(SGE_FONT_LIT("linear"), fcppt::make_cref(*linear_sampler)),
      std::make_pair(SGE_FONT_LIT("mipmap"), fcppt::make_cref(*mipmap_sampler)),
      std::make_pair(SGE_FONT_LIT("trilinear"), fcppt::make_cref(*trilinear_sampler)),
      std::make_pair(
          SGE_FONT_LIT("anisotropic") + anisotropy_string, fcppt::make_cref(*anisotropic_sampler)),
      std::make_pair(
          SGE_FONT_LIT("anisotropic") + anisotropy_string + SGE_FONT_LIT(" + mipmap"),
          fcppt::make_cref(*anisotropic_mipmap_sampler)),
      std::make_pair(
          SGE_FONT_LIT("anisotropic") + anisotropy_string + SGE_FONT_LIT(" + trilinear"),
          fcppt::make_cref(*anisotropic_trilinear_sampler))};
  ...
  using sprite_state_choices = sge::sprite::state::no_choices;
  ...
  sprite_buffers_type sprite_buffers(
      fcppt::make_ref(sys.renderer_device_core()), sge::sprite::buffers::option::static_);

  sprite_state_object sprite_states(
      fcppt::make_ref(sys.renderer_device_core()), sprite_state_parameters());

  sprite_object::unit const sprite_size(
      2000 // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  );

  auto const repetition(fcppt::cast::int_to_float<sprite_object::repetition_type::value_type>(
      sprite_size / 5 // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
      ));

  sprite_object const sprite(
      sge::sprite::roles::pos{} =
          fcppt::math::vector::fill<sprite_object::vector>(-(sprite_size / 2)),
      sge::sprite::roles::size{} = fcppt::math::dim::fill<sprite_object::dim>(sprite_size),
      sge::sprite::roles::repetition{} =
          fcppt::math::vector::fill<sprite_object::repetition_type>(repetition),
          ...
          fcppt::unique_ptr_to_const(fcppt::unique_ptr_to_base<sge::texture::part>(
              fcppt::make_unique_ptr<sge::texture::part_raw_ptr>(
                  sge::renderer::texture::create_planar_from_view(
                      ...
                      sge::image2d::view::const_object(whole_store.const_wrapped_view()),
                      sge::renderer::texture::mipmap::all_levels(
                          sge::renderer::texture::mipmap::auto_generate::yes),
                      sge::renderer::resource_flags_field::null(),
                      sge::renderer::texture::emulate_srgb::no))))});

  using sprite_range_type = sge::sprite::render::range<sprite_choices>;

  sprite_range_type const sprite_range(sge::sprite::geometry::update_one(sprite, sprite_buffers));

  fcppt::reference<filter_array::value_type const> current_filter{fcppt::array::get<0>(filters)};

  auto const key_event(
      [&filters, &current_filter](sge::input::keyboard::event::key const &_event)
      {
        if (!_event.pressed())
        {
          return;
        ...
        fcppt::optional::maybe_void(
            sge::input::key::code_to_digit(_event.get().code()),
            [&filters, &current_filter](sge::input::key::digit const _digit)
            {
              fcppt::optional::maybe_void(
                  fcppt::container::at_optional(
                      filters, filter_array::size_type{_digit.get() - 1U}),
                  [&current_filter](fcppt::reference<filter_array::value_type const> const _filter)
  ...
  sge::font::string const text_appendix(
      ...
      fcppt::output_to_string<sge::font::string>(filters.size()) +
          ...
          sge::renderer::context::ffp &_context, sge::camera::projection_matrix const &_projection)
      {
...

        {
          ...
          sge::renderer::state::core::sampler::scoped_single const scoped_filter(
              ...
              fcppt::make_cref(current_filter.get().second.get()));
        ...
        sge::font::align_h::max_width const font_width(
            fcppt::cast::size<sge::font::unit>(fcppt::cast::to_signed(
                sge::renderer::target::viewport_size(sys.renderer_device_core().onscreen_target())
                    .w())));
        ...
        sge::font::draw::simple(
            ...
            _context,
            *font,
            current_filter.get().first + text_appendix,
  ...
  auto const draw(
      [&frame_timer, &frames_counter, &draw_camera, &camera, &sys]
      {
        if (sge::renderer::target::viewport_is_null(
        ...
        scoped_block.get().clear(sge::renderer::clear::parameters().back_buffer(
            sge::image::color::any::object{sge::image::color::predef::lightblue()}));
  ...
  return sge::window::loop(
      ...
      sge::window::loop_function{
          [&sys, &camera, &key_event, &draw](awl::event::base const &_event)
          {
