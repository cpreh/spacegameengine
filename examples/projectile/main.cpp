...
Container container_from_stream(std::basic_istream<Ch, Traits> &s)
{
  Ch c{};
  ...
  while (c != s.widen(')'))
  {
    ...
    typename Container::value_type new_value{fcppt::no_init()};
    ...
    result.insert(result.end(), new_value);
...
public:
  ...
  body(
      ...
      : body_{sge::projectile::body::parameters{
            _log,
            sge::projectile::body::position(fcppt::math::box::center(_rect)),
            sge::projectile::body::linear_velocity(
                fcppt::math::vector::null<sge::projectile::vector2>()),
            sge::projectile::body::angular_velocity(fcppt::literal<sge::projectile::scalar>(0)),
            std::move(_shape),
            sge::projectile::body::rotation(fcppt::literal<sge::projectile::scalar>(0)),
            _solidity,
            sge::projectile::body::user_data()}},
        body_scope_{_world, fcppt::make_ref(body_), sge::projectile::group::sequence{_group}}
  {
  }
  ...
  [[nodiscard]] sge::projectile::body::object &get() { return body_; }
...
{
  FCPPT_NONMOVABLE(body_keyboard_mover);

public:
  FCPPT_PP_PUSH_WARNING
  FCPPT_PP_DISABLE_VC_WARNING(4355)
  body_keyboard_mover(
      sge::projectile::world_ref const _world, sge::projectile::body::object_ref const _body)
      : body_(_body),
        body_collision_connection_(_world.get().body_collision(
            sge::projectile::body::collision{[this](
                                                 sge::projectile::body::object const &_body1,
                                                 sge::projectile::body::object const &_body2)
                                             { this->body_collision(_body1, _body2); }})),
        velocity_(fcppt::math::vector::null<sge::projectile::vector2>())
  {
  }
  FCPPT_PP_POP_WARNING

  ~body_keyboard_mover() = default;

  void key_event(sge::input::keyboard::event::key const &_event)
  {
    velocity_ = body_.get().linear_velocity() +
                fcppt::literal<sge::projectile::scalar>(
                    30 // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
                    ) *
                    body_keyboard_mover::key_event_to_vector(_event);

    body_.get().linear_velocity(velocity_);
  }
...
private:
  ...
  void body_collision(
      sge::projectile::body::object const &_body1, sge::projectile::body::object const &_body2)
  {
    if (&_body1 == &body_.get() || &_body2 == &body_.get())
    {
      body_.get().linear_velocity(velocity_);

      body_.get().angular_velocity(fcppt::literal<sge::projectile::scalar>(0));
    }
  }

  static sge::projectile::vector2 key_code_to_vector(sge::input::key::code const _key_code)
  {
    FCPPT_PP_PUSH_WARNING
    ...
    case sge::input::key::code::left:
      return sge::projectile::vector2{-1.F, 0.F};
    case sge::input::key::code::right:
      return sge::projectile::vector2{1.F, 0.F};
    case sge::input::key::code::up:
      return sge::projectile::vector2{0.F, -1.F};
    case sge::input::key::code::down:
      return sge::projectile::vector2{0.F, 1.F};
    default:
      return fcppt::math::vector::null<sge::projectile::vector2>();
    }

    FCPPT_PP_POP_WARNING
  }

  static sge::projectile::vector2
  key_event_to_vector(sge::input::keyboard::event::key const &_event)
  {
    sge::projectile::vector2 const result{key_code_to_vector(_event.get().code())};

    return _event.pressed() ? result : -result;
  }
};

void body_collision(sge::projectile::body::object const &, sge::projectile::body::object const &)
{
...
{
  FCPPT_NONMOVABLE(body_following_ghost);
...
public:
  ...
  body_following_ghost(
      ...
      : ghost_(sge::projectile::ghost::parameters(
            _log, sge::projectile::ghost::position(_body.get().position()), _size)),
        ghost_scope_(_world, fcppt::make_ref(ghost_), _groups),
        body_position_change_connection_(
            _body.get().position_change(sge::projectile::body::position_change{
                [this](sge::projectile::body::position const &_pos)
                { ghost_.position(_pos.get()); }})),
        body_enter_connection_(ghost_.body_enter(sge::projectile::ghost::body_enter{
            [](sge::projectile::body::object const &) { std::cout << "Body enter!\n"; }})),
        body_exit_connection_(ghost_.body_exit(sge::projectile::ghost::body_exit{
            [](sge::projectile::body::object const &) { std::cout << "Body exit!\n"; }}))
  {
  }
  FCPPT_PP_POP_WARNING

  ~body_following_ghost() = default;
...
try
{
  sge::systems::instance<
      ...
      sys(sge::systems::make_list(sge::systems::window(sge::systems::window_source(
          sge::systems::original_window(sge::window::title(FCPPT_TEXT("projectile"))))))(
  ...
  fcppt::signal::auto_connection const body_collision_world(
      world.body_collision(sge::projectile::body::collision{&body_collision}));

  sge::projectile::debug_drawer debug_drawer(
      sys.log_context(), fcppt::make_ref(world), fcppt::make_ref(sys.renderer_device_core()));

  sge::projectile::group::object first_group(world);
  sge::projectile::group::object second_group(world);

  world.make_groups_collide(first_group, second_group);

  debug_drawer.active(true);

  body first_body(
      log,
      fcppt::make_ref(world),
      fcppt::make_ref(first_group),
      sge::projectile::shape::shared_base_ptr{
          fcppt::make_shared_ptr<sge::projectile::shape::circle>(
              fcppt::literal<sge::projectile::scalar>(
                  5 // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
                  ))},
      sge::projectile::body::solidity::variant{sge::projectile::body::solidity::solid(
          sge::projectile::body::mass(fcppt::literal<sge::projectile::scalar>(1)))},
      sge::projectile::rect(
  ...
  body second_body(
      ...
      sge::projectile::shape::shared_base_ptr{fcppt::make_shared_ptr<
          ...
          sge::projectile::triangulation::triangulate<
              ...
              container_from_stream<std::vector<sge::projectile::vector2>>(polygon_stream),
              fcppt::literal<sge::projectile::scalar>(
                  0.00001 // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
                  )))},
      sge::projectile::body::solidity::variant{sge::projectile::body::solidity::static_()},
      sge::projectile::rect(
          sge::projectile::vector2(
  ...
  body_following_ghost first_ghost{
      ...
      sge::projectile::ghost::size(sge::projectile::dim2(
          100.F, // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
          100.F // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
          )),
      sge::projectile::group::sequence{fcppt::make_ref(first_group)}};

  body_keyboard_mover keyboard_mover{fcppt::make_ref(world), fcppt::make_ref(first_body.get())};

  sge::timer::basic<sge::timer::clocks::standard> frame_timer(
      sge::timer::parameters<sge::timer::clocks::standard>(std::chrono::seconds(1)));

  auto const draw(
      [&debug_drawer, &frame_timer, &sys, &world]
      {
        world.update_continuous(
            sge::projectile::time_increment(std::chrono::duration_cast<sge::projectile::duration>(
        ...
        fcppt::optional::maybe_void(
            ...
            [&scoped_block, &debug_drawer, &sys](sge::renderer::matrix4 const &_projection)
            {
              ...
              debug_drawer.render(scoped_block.get());
            });
      });
  ...
  return sge::window::loop(
      ...
      sge::window::loop_function{
          [&sys, &keyboard_mover, &draw](awl::event::base const &_event)
          {
            ...
            fcppt::optional::maybe_void(
                ...
                [&keyboard_mover, &draw](auto const &_variant)
                {
                  fcppt::variant::match(
                      _variant,
                      [&keyboard_mover](
                          fcppt::reference<sge::input::keyboard::event::key const> const _key_event)
                      { keyboard_mover.key_event(_key_event.get()); },
