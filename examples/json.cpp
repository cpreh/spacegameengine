  ...
  return fcppt::either::match(
      ...
      [](sge::parse::json::start const &_result)
      {
        ...
        sge::parse::json::find_member_exn<sge::parse::json::int_type>(
            fcppt::make_cref(_result.object().members), "foo");

        // test if this also works with const results
        {
          sge::parse::json::object const &const_result(_result.object());

          sge::parse::json::find_member_exn<sge::parse::json::int_type>(
              fcppt::make_cref(const_result.members), "foo");
        }

        sge::parse::json::output::to_stream(std::cout, _result);
...
{
  fcppt::io::cerr() << FCPPT_TEXT("caugth exception: ") << _exception.string() << FCPPT_TEXT('\n');
