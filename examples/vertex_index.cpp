...
try
{
  ...
  using color_type = sge::renderer::vf::color<sge::image::color::bgra8_format>;
  ...
  auto const make_color(
      ...
      { return sge::image::color::convert<sge::image::color::bgra8_format>(_color); });
      ...
      sge::renderer::vertex::create_buffer_from_vertices<format>(
          ...
          fcppt::array::make(
              ...
              vertex{
                  sge::renderer::vf::labels::pos{} = vec3(1.F, -1.F, 0.F),
                  sge::renderer::vf::labels::color{} =
                      make_color(sge::image::color::predef::red())}))};

  using index_format = sge::renderer::index::format_16;

  sge::renderer::index::buffer_unique_ptr const index_buffer(
      sys.renderer_device_core().create_index_buffer(sge::renderer::index::buffer_parameters(
...

  {
    sge::renderer::index::scoped_lock const iblock(
        fcppt::make_ref(*index_buffer), sge::renderer::lock_mode::writeonly);

    using index_view = sge::renderer::index::view<index_format, sge::renderer::index::nonconst_tag>;

    index_view const view(iblock.value());

    index_view::iterator it(view.begin());

    using index_value_type = index_view::value_type;
  ...
  auto const draw(
      [&index_buffer, &vertex_declaration, &vertex_buffer, &sys]
      {
