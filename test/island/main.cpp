...
void write_image(const sge::image_loader_ptr &image_loader,const space_field &image,const std::string &fn)
{
	boost::scoped_array<sge::color> color_image(new sge::color[image.field_count()]);
	for (space_field::const_iterator i = image.begin(); i != image.end(); ++i)
	{
...
struct island_generator
{
	...
	void apply_cloud_filter()
	{
		...
		cloudy_image = space_field(noise_field.dim());
	...
	const space_field selected() const
	{
		return selected_island;
	}
};

void generate_heightmap_particles(space_field &map)
{
	map.zero();
...
struct binary_combinator : public std::binary_function<space_field::value_type,space_field::value_type,space_field::value_type>
{
	space_field::value_type operator()(const space_field::value_type &t, const space_field::value_type &b) const
	{
		return sge::math::almost_zero(b) ? space_field::value_type(0) : t;
	}
};

void combine_binary(space_field &target,const space_field &binary)
{
	assert(target.dim() == binary.dim());
	std::transform(target.begin(),target.end(),binary.begin(),target.begin(),binary_combinator());
}

void generate_heightmap_hills(space_field &map,const space_field &binary_map)
{
	map.zero();

	const unsigned iterations = 800;
	space_field::value_type max_height = 0,min_height = std::numeric_limits<space_field::value_type>::max();
	for (unsigned i = 0; i < iterations; ++i)
	{
		const space_field::value_type radius = (rand() / (RAND_MAX + 1.0)) * space_field::value_type(8);
		//const space_field::value_type theta = (rand() / (RAND_MAX+1.0))*2*sge::math::PI;
		...
		for (unsigned x = 0; x < map.width(); ++x)
		{
			...
			{
				const space_field::value_type x_diff = x-space_field::value_type(current_field.x());
				const space_field::value_type y_diff = y-space_field::value_type(current_field.y());
				const space_field::value_type value = radius*radius - ((x_diff*x_diff)+(y_diff*y_diff));
	...
	for (space_field::iterator i = map.begin(); i != map.end(); ++i)
	{
...
int main()
{
	...
	space_field m(generator.selected().dim());
