...
template <typename DestFormat, typename Src>
sge::image::pixel::mizuiro_type<DestFormat> convert(Src const &_src)
{
  static_assert(
      sge::image::color::is_convertible<typename Src::format, DestFormat>::value,
      "Formats are not convertible");

  return mizuiro::color::convert_static::convert(fcppt::tag<DestFormat>{}, _src);
}
