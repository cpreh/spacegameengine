...
sge::texture::part_unique_ptr sge::texture::wrap_npot(
    ...
    sge::renderer::texture::planar_parameters const &_parameters)
{
  ...
  sge::renderer::dim2 const padded_size{fcppt::math::dim::map(
      _parameters.size(), [](sge::renderer::size_type const _sz) { return _sz == 0U ? 1U : _sz; })};

  return fcppt::unique_ptr_to_base<sge::texture::part>(
      fcppt::make_unique_ptr<sge::texture::part_raw_ptr>(
          _device.get().create_planar_texture(sge::renderer::texture::planar_parameters(
              _device.get().caps().non_power_of_2_textures().get()
                  ? padded_size
                  : sge::texture::next_power_of_2(padded_size),
              _parameters.format(),
              _parameters.mipmap(),
              _parameters.resource_flags(),
              _parameters.capabilities())),
          sge::renderer::lock_rect(
              fcppt::math::vector::null<sge::renderer::lock_rect::vector>(), _parameters.size())));
}
