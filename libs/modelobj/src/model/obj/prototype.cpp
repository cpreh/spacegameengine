...
using line_count = std::uint64_t;

sge::model::obj::face_vertex
parse_face_vertex(line_count const _current_line, std::istream &_stream)
{
  ...
  if (!(_stream >> vertex_coordinate_index))
  {
    throw sge::model::obj::exception(
        FCPPT_TEXT("Error on line ") + fcppt::output_to_fcppt_string(_current_line) +
  ...
  return sge::model::obj::face_vertex(
      sge::model::obj::vertex_coordinate_index(vertex_coordinate_index - 1U),
      sge::model::obj::texture_coordinate_index(texture_coordinate_index - 1U),
      sge::model::obj::normal_index(normal_index - 1U));
}
}
...
sge::model::obj::prototype::prototype(
    ...
    : log_{_log_context, sge::log::location(), sge::log::default_parameters(sge::model::obj::impl::log_name())},
      ...
      parts_(),
      material_files_(),
      bounding_box_(fcppt::math::box::null<sge::model::obj::box>())
{
  ...
  using face_vertex_to_index_map = std::unordered_map<sge::model::obj::face_vertex, std::size_t>;

  std::filesystem::path const _obj_file_path(_filename.parent_path());

  face_vertex_to_index_map face_vertex_to_index;

  line_count line_counter = 0U;

  sge::model::obj::identifier current_material{sge::charconv::utf8_string{}};
  ...
  while (std::getline(stream, line))
  {
    ...
    {
      throw sge::model::obj::exception(
          FCPPT_TEXT("Error on line ") + fcppt::output_to_fcppt_string(line_counter) +
    ...
    if (prefix == "mtllib")
    {
      material_files_.push_back(_obj_file_path / rest_of_line);
    }
    else if (prefix == "usemtl")
    {
      current_material = sge::model::obj::identifier{
          sge::charconv::utf8_string{rest_of_line.begin(), rest_of_line.end()}};
    }
    else if (prefix == "v")
    {
      std::istringstream line_rest(rest_of_line);

      sge::model::obj::vertex_coordinate coordinate{fcppt::no_init()};
      ...
      {
        bounding_box_ =
            sge::model::obj::box(coordinate, fcppt::math::dim::null<sge::model::obj::box::dim>());
      }
      else
      {
        bounding_box_ = fcppt::math::box::extend_bounding_box(bounding_box_, coordinate);
      }

      vertex_coordinates_.push_back(coordinate);
    }
    else if (prefix == "vn")
    {
      std::istringstream line_rest(rest_of_line);

      sge::model::obj::normal normal{fcppt::no_init()};
    ...
    else if (prefix == "vt")
    {
      ...
      sge::model::obj::texture_coordinate tex_coord{fcppt::no_init()};
    ...
    else if (prefix == "f")
    {
      ...
      using face_vertex_array = fcppt::array::object<sge::model::obj::face_vertex, 3>;

      face_vertex_array const temporary_face_vertices{
          parse_face_vertex(line_counter, line_rest),
          parse_face_vertex(line_counter, line_rest),
          parse_face_vertex(line_counter, line_rest)};

      sge::model::obj::face face{fcppt::no_init{}};

      for (std::size_t i = 0; i < temporary_face_vertices.size(); ++i)
      {
        auto it = face_vertex_to_index.find(temporary_face_vertices.get_unsafe(i));
        ...
        {
          face_vertices_.push_back(temporary_face_vertices.get_unsafe(i));

          it = face_vertex_to_index
                   .insert(std::make_pair(
                       temporary_face_vertices.get_unsafe(i),
                       static_cast<std::size_t>(face_vertices_.size() - 1U)))
                   .first;
        }
      ...
      parts_[current_material].push_back(face);
    }
    else
    {
      FCPPT_LOG_WARNING(
          log_,
          fcppt::log::out << FCPPT_TEXT("Invalid obj prefix: ") << fcppt::from_std_string(prefix))
    }
  }

  if (texture_coordinates_.empty())
  {
    texture_coordinates_.push_back(fcppt::math::vector::null<sge::renderer::vector2>());
  }
}
...
sge::model::obj::normal_sequence const &sge::model::obj::prototype::normals() const
{
  return normals_;
}

sge::model::obj::face_vertex_sequence const &sge::model::obj::prototype::face_vertices() const
{
  return face_vertices_;
}

sge::model::obj::material_to_face_sequence const &sge::model::obj::prototype::parts() const
{
  return parts_;
}

std::size_t sge::model::obj::prototype::face_count() const
{
...
sge::model::obj::material::file_sequence const &sge::model::obj::prototype::material_files() const
{
  return material_files_;
}

sge::model::obj::box const &sge::model::obj::prototype::bounding_box() const
{
  return bounding_box_;
}

sge::model::obj::prototype::~prototype() = default;
