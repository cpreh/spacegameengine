...
sge::input::cursor::relative_movement::detail::object::object(
    ...
    : positions_{fcppt::algorithm::map<position_map>(
          _processor.get().cursors(),
          [](sge::input::cursor::shared_ptr const &_cursor)
          { return std::make_pair(_cursor, _cursor->position()); })},
      connection_{_processor.get().window().event_handler(sge::window::event_function{
          [this](awl::window::event::base const &_event) -> awl::event::container
...
sge::input::cursor::relative_movement::detail::object::~object() = default;

awl::event::container sge::input::cursor::relative_movement::detail::object::on_event(
    awl::window::event::base const &_event)
{
  return fcppt::optional::to_container<awl::event::container>(fcppt::optional::bind(
      fcppt::cast::dynamic<sge::input::cursor::event::move const>(_event),
      [this](fcppt::reference<sge::input::cursor::event::move const> const _move_event)
      {
        return fcppt::optional::map(
            this->update_pos(_move_event.get()),
            [_move_event](sge::input::cursor::relative_movement::difference const &_difference)
            {
              return fcppt::unique_ptr_to_base<awl::event::base>(
                  fcppt::make_unique_ptr<sge::input::cursor::relative_movement::event>(
                      _move_event.get().cursor(), _difference));
            });
      }));
}
...
{
  optional_difference difference{fcppt::optional::apply(
      [](sge::input::cursor::position const &_old_pos, sge::input::cursor::position const &_new_pos)
      { return sge::input::cursor::relative_movement::difference{_old_pos - _new_pos}; },
      this->positions_[_move_event.cursor()],
      _move_event.position())};

  this->positions_[_move_event.cursor()] = _move_event.position();
