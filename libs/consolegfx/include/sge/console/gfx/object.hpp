...
public:
  ...
  SGE_CONSOLE_GFX_DETAIL_SYMBOL
  void focus_event(sge::input::focus::event::base const &);

  SGE_CONSOLE_GFX_DETAIL_SYMBOL
  void area(sge::font::rect const &);

  [[nodiscard]] SGE_CONSOLE_GFX_DETAIL_SYMBOL sge::console::object &console_object();

  [[nodiscard]] SGE_CONSOLE_GFX_DETAIL_SYMBOL sge::console::object const &console_object() const;

private:
  using input_history_sequence = std::list<sge::font::string>;
  ...
  void print(sge::font::string &&);

  [[nodiscard]] sge::font::unit render_line(
      sge::renderer::context::ffp &, // NOLINT(google-runtime-references)
      sge::font::string const &,
      sge::font::unit);

  void on_key(sge::input::focus::event::key const &);

  void on_text(sge::input::focus::event::text const &);

  void on_key_repeat(sge::input::focus::event::key_repeat const &);

  void key_action(sge::input::focus::shared_ptr const &, sge::input::focus::key const &);

  void error(sge::font::string &&);
};
