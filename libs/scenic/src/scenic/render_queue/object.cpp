    ...
    fcppt::reference<StateSequence> const _states, typename StateSequence::value_type _state)
{
  ...
  {
    _states.get().push_back(_state);

    it = std::prev(_states.get().end());
  }

  auto begin(_states.get().cbegin());

  return static_cast<sge::scenic::render_queue::index_type>(std::distance(begin, it));
}
}

sge::scenic::render_queue::object::object(sge::scenic::texture_manager_ref const _texture_manager)
    : texture_manager_(_texture_manager),
      materials_(),
      vertex_buffers_(),
      meshes_(),
      current_material_(static_cast<sge::scenic::render_queue::index_type>(-1)),
      current_vertex_buffer_(static_cast<sge::scenic::render_queue::index_type>(-1))
{
}
...
void sge::scenic::render_queue::object::current_vertex_buffer(
    sge::renderer::vertex::buffer_ref const _vertex_buffer)
{
  current_vertex_buffer_ =
      change_context_state(fcppt::make_ref(vertex_buffers_), &_vertex_buffer.get());
}

void sge::scenic::render_queue::object::add_mesh(
...
sge::scenic::render_queue::object::render(sge::scenic::render_context::base &_context)
{
  std::sort(meshes_.begin(), meshes_.end());

  auto const invalid_index = static_cast<index_type>(-1);

  index_type current_render_material{invalid_index};

  index_type current_render_vertex_buffer{invalid_index};

  sge::scenic::render_queue::state_change_count state_changes(0U);

  for (auto const &current_mesh : meshes_)
  {
    if (current_mesh.material() != current_render_material)
    {
      ...
      _context.material(
          materials_[static_cast<material_sequence::size_type>(current_mesh.material())]);

      current_render_material = current_mesh.material();
    }
    ...
    if (current_mesh.vertex_buffer() != current_render_vertex_buffer)
    {
      ...
      _context.vertex_buffer(*vertex_buffers_[static_cast<vertex_buffer_sequence::size_type>(
          current_mesh.vertex_buffer())]);

      current_render_vertex_buffer = current_mesh.vertex_buffer();
    }

    state_changes++;
    _context.transform(
        sge::scenic::render_context::transform_matrix_type::world, current_mesh.modelview());

    _context.render(current_mesh.index_buffer(), current_mesh.index_buffer_range());
  }

  return state_changes;
}

sge::scenic::render_queue::object::~object() = default;
