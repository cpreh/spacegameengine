...
bool compare_floats(float const _a, float const _b)
{
...
struct compare_channels
{
  ...
  bool operator()(float const _a, float const _b) const { return compare_floats(_a, _b); }
};

template <typename T>
bool compare_colors(T const &_a, T const &_b)
{
  return sge::image::color::any::compare(_a.get(), _b.get(), compare_channels());
}
...
sge::scenic::render_context::material::object::object(
    ...
    : diffuse_color_(std::move(_diffuse_color)),
      ...
      diffuse_texture_(_diffuse_texture),
      specular_texture_(_specular_texture)
{
}
...
bool sge::scenic::render_context::material::object::operator==(
    sge::scenic::render_context::material::object const &_right) const
{
  return compare_colors(diffuse_color_, _right.diffuse_color_) &&
         compare_colors(ambient_color_, _right.ambient_color_) &&
         compare_colors(specular_color_, _right.specular_color_) &&
         compare_colors(emissive_color_, _right.emissive_color_) &&
         compare_floats(shininess_.get(), _right.shininess_.get()) &&
         diffuse_texture_ == _right.diffuse_texture_ &&
         specular_texture_ == _right.specular_texture_;
}
