...
sge::scenic::render_context::cg::object::object(
    fcppt::reference<sge::scenic::render_context::cg::manager> const _manager,
    sge::renderer::context::core_ref const _context)
    : sge::scenic::render_context::base(),
      manager_(_manager),
      context_(_context),
      scoped_vd_(_context, _manager.get().vertex_declaration_),
      depth_stencil_state_(_context, fcppt::make_cref(*manager_.get().depth_stencil_state_)),
      blend_state_(_context, fcppt::make_cref(*manager_.get().blend_state_)),
      rasterizer_state_(_context, fcppt::make_cref(*manager_.get().rasterizer_state_)),
      scoped_shader_(_context, fcppt::make_ref(manager_.get().shader_)),
      sampler_state_{
          _context,
          sge::renderer::state::core::sampler::const_object_ref_map{
              ...
              sge::renderer::state::core::sampler::const_object_ref_map::value_type{
                  ...
                  sge::renderer::state::core::sampler::const_object_ref{
                      *manager_.get().mipmap_sampler_state_}}}},
      current_world_(fcppt::math::matrix::identity<sge::renderer::matrix4>()),
      current_projection_(fcppt::math::matrix::identity<sge::renderer::matrix4>()),
      current_vertex_buffer_()
{
}

void sge::scenic::render_context::cg::object::transform(
    ...
    sge::renderer::matrix4 const &_matrix)
{
  ...
  case sge::scenic::render_context::transform_matrix_type::world:
    current_world_ = _matrix;

    manager_.get().world_matrix_.set(current_world_);

    manager_.get().world_inverse_transpose_matrix_.set(
        fcppt::math::matrix::inverse(fcppt::math::matrix::transpose(current_world_)));
    ...
    sge::scenic::render_context::material::object const &_material)
{
  manager_.get().material_diffuse_color_.set(
      sge::scenic::impl::render_context::cg::any_color_to_vector4(_material.diffuse_color().get()));
  ...
  manager_.get().material_ambient_color_.set(
      sge::scenic::impl::render_context::cg::any_color_to_vector4(_material.ambient_color().get()));
...
void sge::scenic::render_context::cg::object::lights(
    sge::scenic::render_context::light::sequence const &_lights)
{
  ...
  for (sge::scenic::render_context::light::object const &light : _lights)
  {
    // TODO(philipp): This should be done using a visitor!
    fcppt::optional::maybe_void(
        fcppt::variant::to_optional<sge::scenic::render_context::light::point>(light.variant()),
        [&light, &point_light_count, this](
            sge::scenic::render_context::light::point const &_point_light)
        {
          sge::scenic::render_context::cg::light::point &current_light(
              *manager_.get().point_lights_.get_unsafe(
                  static_cast<std::size_t>(point_light_count++)));

          current_light.diffuse_color(light.diffuse_color());

          current_light.specular_color(light.specular_color());

          current_light.ambient_color(light.ambient_color());

          current_light.camera_space_position(
              fcppt::math::matrix::transform_point(current_world_, _point_light.position().get()));

          current_light.attenuation(_point_light.attenuation());
        });

    fcppt::optional::maybe_void(
        fcppt::variant::to_optional<sge::scenic::render_context::light::directional>(
            light.variant()),
        [&light, &directional_light_count, this](
            sge::scenic::render_context::light::directional const &_directional_light)
        {
          sge::scenic::render_context::cg::light::directional &current_light(
              *manager_.get().directional_lights_.get_unsafe(
                  static_cast<std::size_t>(directional_light_count++)));
          ...
          current_light.camera_space_direction(sge::scenic::render_context::light::direction(
              fcppt::math::vector::narrow_cast<sge::renderer::vector3>(
                  current_world_ * fcppt::math::vector::push_back(
                                       _directional_light.direction().get(),
                                       static_cast<sge::renderer::scalar>(0.0F)))));
        });
  }

  manager_.get().point_light_count_.set(point_light_count);

  manager_.get().directional_light_count_.set(directional_light_count);
}

void sge::scenic::render_context::cg::object::vertex_buffer(
    sge::renderer::vertex::buffer const &_new_vertex_buffer)
{
  fcppt::optional::maybe_void(
      current_vertex_buffer_,
      [this](fcppt::reference<sge::renderer::vertex::buffer const> const _vertex_buffer)
      { context_.get().deactivate_vertex_buffer(_vertex_buffer.get()); });

  current_vertex_buffer_ = optional_vertex_buffer(fcppt::make_cref(_new_vertex_buffer));

  context_.get().activate_vertex_buffer(fcppt::make_cref(_new_vertex_buffer));
}

void sge::scenic::render_context::cg::object::fog(
    sge::scenic::render_context::fog::optional_properties const &_opt_fog)
{
  manager_.get().use_fog_.set(_opt_fog.has_value());

  fcppt::optional::maybe_void(
      _opt_fog,
      [this](sge::scenic::render_context::fog::properties const &_fog)
      {
        manager_.get().fog_start_.set(_fog.start().get());
        manager_.get().fog_end_.set(_fog.end().get());
        manager_.get().fog_color_.set(
            sge::scenic::impl::render_context::cg::any_color_to_vector4(_fog.color().get()));
      });
}

void sge::scenic::render_context::cg::object::render(
...
sge::renderer::target::base &sge::scenic::render_context::cg::object::target()
{
