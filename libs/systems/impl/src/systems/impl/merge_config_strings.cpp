...
sge::systems::optional_name sge::systems::impl::merge_config_strings(
...
{
  return _parameter_name.has_value()
             ? _parameter_name
             : fcppt::optional::map(
                   fcppt::optional::bind(
                       _ini_config,
                       [&_entry_name](sge::parse::ini::start const &_start)
                       {
                         return sge::parse::ini::find_value(
                             _start, sge::systems::impl::ini_section_name(), _entry_name);
                       }),
                   [](sge::parse::ini::value const &_ini_value)
                   { return sge::plugin::name(fcppt::from_std_string(_ini_value.get())); });
}
