...
sge::renderer::device::core_unique_ptr sge::systems::impl::renderer::create_device(
    ...
    sge::systems::impl::window::object const &_window)
{
  sge::renderer::device::parameters const parameters(
      _parameters.parameters().display_mode(), fcppt::make_ref(_window.get()));

  return (_parameters.caps() == sge::systems::renderer_caps::ffp)
             ? fcppt::unique_ptr_to_base<sge::renderer::device::core>(
                   _system.get().create_ffp_renderer(parameters))
             : _system.get().create_core_renderer(parameters);
}
