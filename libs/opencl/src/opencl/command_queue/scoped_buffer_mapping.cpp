...
sge::opencl::command_queue::scoped_buffer_mapping::scoped_buffer_mapping(
    ...
    : queue_(_queue), buffer_(_buffer.get().impl()), ptr_(nullptr)
{
  cl_int error_code{};

  ptr_ = clEnqueueMapBuffer(
      _queue.get().impl(),
      buffer_,
      // Blocking: yes
      CL_TRUE,
  ...
  opencl::impl::handle_error(error_code, FCPPT_TEXT("clEnqueueMapBuffer"));
}

void *sge::opencl::command_queue::scoped_buffer_mapping::ptr() const { return ptr_; }
...
sge::opencl::command_queue::scoped_buffer_mapping::~scoped_buffer_mapping()
{
  ...
  cl_int const error_code =
      clEnqueueUnmapMemObject(queue_.get().impl(), buffer_, this->ptr(), 0, nullptr, nullptr);
