...
sge::opencl::kernel::object::object(
    sge::opencl::program::object_ref const _program, sge::opencl::kernel::name &&_name)
      ...
      argument_count_()
{
  ...
  kernel_ = clCreateKernel(_program.get().program_, this->name_.get().c_str(), &error_code);

  if (error_code == CL_INVALID_KERNEL_NAME)
  {
    throw sge::opencl::exception(
        FCPPT_TEXT("The kernel name \"") + fcppt::from_std_string(this->name_.get()) +
    ...
    sge::opencl::kernel::argument_index const &index, sge::opencl::memory_object::base_ref const _memory)
{
  ...
  {
    throw sge::opencl::exception{FCPPT_TEXT("Kernel argument index is out of range")};
  }
...
{
  fcppt::variant::apply(
      [_index, this](auto const _value)
      {
        cl_int const error_code{::clSetKernelArg(
            kernel_, fcppt::cast::size<cl_uint>(_index.get()), sizeof(decltype(_value)), &_value)};
        ...
        case CL_INVALID_ARG_SIZE:
          throw sge::opencl::exception{
              FCPPT_TEXT("Kernel \"") + fcppt::from_std_string(name_.get()) +
              FCPPT_TEXT("\": clSetKernelArg for argument ") +
              fcppt::output_to_fcppt_string(_index) +
