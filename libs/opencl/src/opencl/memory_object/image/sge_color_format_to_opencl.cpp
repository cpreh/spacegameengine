...
sge::opencl::memory_object::image::sge_color_format_to_opencl(sge::image::color::format const f)
{
  ...
  case sge::image::color::format::a8:
    return make_cl_image_format(CL_A, CL_UNORM_INT8);
  case sge::image::color::format::l8:
    return make_cl_image_format(CL_INTENSITY, CL_UNORM_INT8);
  case sge::image::color::format::r32f:
    return make_cl_image_format(CL_R, CL_FLOAT);
  case sge::image::color::format::la8:
    return make_cl_image_format(CL_RA, CL_UNORM_INT8);
  ...
  case sge::image::color::format::sbgr8:
    return make_cl_image_format(CL_RGB, CL_UNORM_INT8);
  case sge::image::color::format::rgba8:
  // CL doesn't seem to support sRGB directly
  case sge::image::color::format::srgba8:
  case sge::image::color::format::rgbx8: // TODO(philipp): map to CL_RGBx?
    return make_cl_image_format(CL_RGBA, CL_UNORM_INT8);
  case sge::image::color::format::bgra8:
  case sge::image::color::format::sbgra8:
  case sge::image::color::format::bgrx8: // TODO(philipp): would CL_RGBx be better?
    return make_cl_image_format(CL_BGRA, CL_UNORM_INT8);
  case sge::image::color::format::rgb32f:
  case sge::image::color::format::bgr32f:
    return make_cl_image_format(CL_RGB, CL_FLOAT);
