...
sge::opencl::program::build_parameters::build_parameters()
    : devices_(), build_options_(), notification_callback_()
{
}

sge::opencl::program::build_parameters sge::opencl::program::build_parameters::devices(
    sge::opencl::device::object_ref_sequence &&_devices) &&
{
  devices_ = optional_object_ref_sequence(std::move(_devices));

  return std::move(*this);
}

sge::opencl::program::build_parameters sge::opencl::program::build_parameters::options(
    sge::opencl::program::build_options &&_build_options) &&
{
  build_options_ = std::move(_build_options);
    ...
    sge::opencl::program::notification_callback &&_notification_callback) &&
{
  notification_callback_ =
      sge::opencl::program::optional_notification_callback{std::move(_notification_callback)};
