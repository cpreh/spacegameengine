...
{
  FCPPT_NONMOVABLE(scoped_texture);

public:
  SGE_RENDERER_CG_DETAIL_SYMBOL
  scoped_texture(sge::renderer::context::core_ref, sge::renderer::cg::const_loaded_texture_ref);

  SGE_RENDERER_CG_DETAIL_SYMBOL
  ~scoped_texture();

  [[nodiscard]] SGE_RENDERER_CG_DETAIL_SYMBOL sge::renderer::texture::stage stage() const;
