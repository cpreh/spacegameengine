...
sample(fcppt::reference<Noise> const _noise, sample_parameters<Noise> const &_parameters)
{
  ...
  double const amp{static_cast<double>(_parameters.amplitude().get())};
  double const freq{static_cast<double>(_parameters.frequency().get())};
  double const octaves{static_cast<double>(_parameters.octaves().get())};
  double const scaling{std::pow(2.0, octaves) / (std::pow(2.0, octaves) - 1.0)};

  for (typename sge::noise::sample_parameters<Noise>::octaves_type i(1U);
       ...
       ++i)
  {
    sum +=
        ...
        static_cast<double>(_noise.get().sample(
  ...
  return static_cast<result_type>(sum * amp * scaling);
}
