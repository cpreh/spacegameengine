...
public:
  ...
  explicit object(sge::noise::simplex::width const &);

  [[nodiscard]] Float sample(vector_type const &);

private:
  using index_type = fcppt::math::size_type;
  ...
  mod(long int const &a, // NOLINT(google-runtime-int)
      std::size_t const &b);

  [[nodiscard]] std::size_t index(vector_type const &vec);

  using corner_array = fcppt::array::object<vector_type, N + 1>;

  [[nodiscard]] corner_array corners(vector_type point);

  [[nodiscard]] Float stretch_factor();

  [[nodiscard]] Float inv_factor();

  [[nodiscard]] matrix stretch_m();

  [[nodiscard]] matrix inv_m();

  [[nodiscard]] Float contrib(vector_type const &v, vector_type const &intv);
};
