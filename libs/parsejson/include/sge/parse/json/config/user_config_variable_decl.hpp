...
{
  FCPPT_NONCOPYABLE(user_config_variable);

public:
  typedef void callback_fn(T const &);

  typedef fcppt::function<callback_fn> callback;

  user_config_variable(
      sge::parse::json::object const &, sge::parse::json::object &, sge::parse::json::path const &);

  void value(T const &);

  T const &value() const;

  fcppt::signal::auto_connection change_callback(callback const &);
