...
Container to_static_container(sge::parse::json::array const &_array)
{
  // TODO(philipp): array::from_container
  if (_array.elements.size() !=
      static_cast<sge::parse::json::element_vector::size_type>(Container::dim_wrapper::value))
  {
    throw sge::parse::exception(
        FCPPT_TEXT("convert::to_static_container: unequal amount of elements!"));
  }

  Container result{fcppt::no_init()};
