...
sge::console::muxing_streambuf<Char, Traits>::muxing_streambuf(
    fcppt::reference<std::basic_ostream<Char, Traits>> const _stream,
    ...
    : streambuf_base(),
      ...
      object_(_object),
      stream_(_stream),
      old_streambuf_(stream_.get().rdbuf(this)),
      does_muxing_(_does_muxing),
      buffer_()
{
}
...
{
template <typename Char, typename Traits>
muxing_streambuf<Char, Traits>::~muxing_streambuf()
{
  stream_.get().rdbuf(old_streambuf_.release_ownership());
}
}
...
sge::console::muxing_streambuf<Char, Traits>::overflow(int_type const _char)
{
  if (in_progress_)
  {
    return streambuf_base::traits_type::not_eof(_char);
  }
  ...
  {
    old_streambuf_->sputc(streambuf_base::traits_type::to_char_type(_char));
  }

  if (_char != streambuf_base::traits_type::eof())
  {
    if (streambuf_base::traits_type::to_char_type(_char) == stream_.get().widen('\n'))
    {
      object_.get().emit_message(sge::console::impl::from_string(buffer_));

      buffer_.clear();
    }
    else
    {
      buffer_ += streambuf_base::traits_type::to_char_type(_char);
    }
  }
