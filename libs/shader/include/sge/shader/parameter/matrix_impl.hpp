...
sge::shader::parameter::matrix<ValueType, M, N>::matrix(
    ...
    : parameter_(_program.get().parameter(_name.get())),
      is_projection_matrix_(_is_projection_matrix.get() && _renderer.caps().normalized_cvv().get())
{
  this->set(_initial_value);
}

template <typename ValueType, fcppt::math::size_type M, fcppt::math::size_type N>
void sge::shader::parameter::matrix<ValueType, M, N>::set(matrix_type const &_matrix)
{
  ...
  sge::cg::parameter::matrix::set(
      parameter_.object(),
      is_projection_matrix_.get() ? fcppt::math::matrix::translation(zero, zero, -one) *
...
{
template <typename ValueType, fcppt::math::size_type M, fcppt::math::size_type N>
matrix<ValueType, M, N>::~matrix() = default;
}
