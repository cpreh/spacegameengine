...
{
  fcppt::variant::apply(
      [&_src, &_dest, _overlap, _uninitialized](auto const &_src_inner)
      {
        return mizuiro::image::algorithm::copy(
            _src_inner,
            fcppt::optional::to_exception(
                fcppt::variant::to_optional<sge::image::view::mizuiro_type<
                    typename std::decay_t<decltype(_src_inner)>::format,
                    mizuiro::nonconst_tag>>(_dest.get()),
                [&_src, &_dest]
                {
                  return sge::image::invalid_copy<sge::image::traits::image::color_tag<Tag>>{
                      sge::image::view::format<Tag>(_src), sge::image::view::format<Tag>(_dest)};
                }),
