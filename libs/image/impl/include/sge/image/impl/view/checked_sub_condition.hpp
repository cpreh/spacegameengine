...
void checked_sub_condition(View const &_src, sge::image::box<Tag> const &_box) noexcept(false)
{
  using box = sge::image::box<Tag>;

  box const outer(
      fcppt::math::vector::null<typename box::vector>(), sge::image::view::size<Tag>(_src));

  if (!fcppt::math::box::contains(outer, _box))
  {
    throw sge::image::view::sub_out_of_range<Tag>{outer, _box};
  }
}
