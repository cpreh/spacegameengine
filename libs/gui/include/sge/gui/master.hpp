...
class master
{
  FCPPT_NONMOVABLE(master);

public:
  SGE_GUI_DETAIL_SYMBOL
  master(sge::gui::context_ref, sge::gui::main_area::reference);
...
private:
  ...
  void key_repeat_event(sge::input::focus::event::key_repeat const &);

  void text_event(sge::input::focus::event::text const &);

  void button_event(sge::input::cursor::event::button const &);

  void handle_key(sge::input::key::code);

  [[nodiscard]] sge::gui::widget::optional_ref try_focus(sge::gui::widget::optional_focus);
