    ...
    : sge::gui::widget::base(),
      ...
      layout_{sge::rucksack::axis_policy2{
          sge::rucksack::axis_policy{sge::rucksack::preferred_size{
              static_text_.logical_size().w() + style_.get().text_spacing().w()}},
          sge::rucksack::axis_policy{sge::rucksack::preferred_size{
              _font.get().metrics().height().get() + style_.get().text_spacing().h()}}}}
{
}

sge::gui::widget::static_text::~static_text() = default;

sge::rucksack::widget::base &sge::gui::widget::static_text::layout() { return layout_; }

void sge::gui::widget::static_text::on_draw(
    sge::gui::renderer::base &_renderer, sge::renderer::context::ffp &_context)
{
  ...
  static_text_.pos(fcppt::math::vector::structure_cast<sge::font::vector, fcppt::cast::size_fun>(
      this->layout().position() + (style_.get().text_spacing() / 2).get_unsafe()));
