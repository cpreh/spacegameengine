...
{
  FCPPT_NONMOVABLE(const_basic_scoped_lock);

public:
  using texture_type = sge::renderer::texture::basic_lockable_box<Tag>;

  using texture_ref = fcppt::reference<texture_type const>;

  using buffer = sge::renderer::buffer::readable<Tag>;

  using lock_area = sge::image::box<Tag>;

  using const_view = sge::image::view::const_object<Tag>;

  SGE_RENDERER_DETAIL_SYMBOL
  explicit const_basic_scoped_lock(texture_ref);

  SGE_RENDERER_DETAIL_SYMBOL
  const_basic_scoped_lock(texture_ref, lock_area const &);

  SGE_RENDERER_DETAIL_SYMBOL
  const_view value() const;

  SGE_RENDERER_DETAIL_SYMBOL
  ~const_basic_scoped_lock();

private:
  using buffer_lock = sge::renderer::buffer::const_scoped_lock<Tag>;
