...
struct texpos : sge::renderer::vf::vector_base<Format, NumSubElements>
{
  static_assert(
      sge::renderer::is_valid_float_type<Format>::value, "Format must be float or double");

  static_assert(
      NumSubElements >= 2 && NumSubElements <= 3, "NumSubElements mut be between 2 and 3");

  using index = Index;

  using label = sge::renderer::vf::labels::texpos<Index::value>;
};
