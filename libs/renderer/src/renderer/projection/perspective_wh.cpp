...
sge::renderer::matrix4 sge::renderer::projection::perspective_wh(
    ...
    sge::renderer::projection::far const _far)
{
  ...
  sge::renderer::scalar const h(_dim.h());
  sge::renderer::scalar const w(_dim.w());

  return sge::renderer::matrix4(
      fcppt::math::matrix::row(two * near / w, zero, zero, zero),
      fcppt::math::matrix::row(zero, two * near / h, zero, zero),
      fcppt::math::matrix::row(zero, zero, far / (far - near), near * far / (near - far)),
