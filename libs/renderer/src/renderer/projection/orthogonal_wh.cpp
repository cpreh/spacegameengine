...
sge::renderer::matrix4 sge::renderer::projection::orthogonal_wh(
    ...
    sge::renderer::projection::far const _far)
{
  ...
  sge::renderer::scalar const width(_dim.w());
  sge::renderer::scalar const height(_dim.h());
  sge::renderer::scalar const near(_near.get());
  sge::renderer::scalar const far(_far.get());

  return sge::renderer::matrix4(
      fcppt::math::matrix::row(two / width, zero, zero, -one),
      fcppt::math::matrix::row(zero, -two / height, zero, one),
