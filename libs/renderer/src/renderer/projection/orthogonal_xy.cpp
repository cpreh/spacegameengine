...
sge::renderer::matrix4 sge::renderer::projection::orthogonal_xy()
{
  sge::renderer::scalar const one(1.F);
  sge::renderer::scalar const zero(0.F);

  return sge::renderer::matrix4(
      fcppt::math::matrix::row(one, zero, zero, zero),
      fcppt::math::matrix::row(zero, one, zero, zero),
      fcppt::math::matrix::row(zero, zero, zero, zero),
