...
#include <sge/renderer/target/base.hpp> // NOLINT(misc-include-cleaner)
...
sge::renderer::target::scoped_scissor_area::scoped_scissor_area(
    ...
    : target_(_target), old_area_(target_.get().scissor_area())
{
  target_.get().scissor_area(_new_scissor_area);
}

sge::renderer::target::scoped_scissor_area::~scoped_scissor_area()
{
  target_.get().scissor_area(old_area_);
}
