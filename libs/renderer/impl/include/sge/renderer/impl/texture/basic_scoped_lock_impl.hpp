...
sge::renderer::texture::basic_scoped_lock<Tag>::basic_scoped_lock(
    texture_ref const _texture, sge::renderer::lock_mode const _mode)
    : lock_(fcppt::make_ref(_texture.get().level(sge::renderer::texture::mipmap::level(0U))), _mode)
{
}

template <typename Tag>
sge::renderer::texture::basic_scoped_lock<Tag>::basic_scoped_lock(
    texture_ref const _texture, lock_area const &_area, sge::renderer::lock_mode const _mode)
    : lock_(
...
{
template <typename Tag>
basic_scoped_lock<Tag>::~basic_scoped_lock() = default;
}
