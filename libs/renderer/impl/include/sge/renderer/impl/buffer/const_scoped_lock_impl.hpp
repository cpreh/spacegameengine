...
sge::renderer::buffer::const_scoped_lock<Tag>::const_scoped_lock(buffer_ref const _buffer)
    : buffer_(_buffer), view_(buffer_.get().lock_c())
{
}

template <typename Tag>
sge::renderer::buffer::const_scoped_lock<Tag>::const_scoped_lock(
    buffer_ref const _buffer, lock_area const &_area)
    : buffer_(_buffer), view_(buffer_.get().lock_c(_area))
{
}
...
{
template <typename Tag>
const_scoped_lock<Tag>::~const_scoped_lock()
{
  buffer_.get().unlock();
}
}
