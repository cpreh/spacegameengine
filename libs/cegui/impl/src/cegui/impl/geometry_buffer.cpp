...
sge::cegui::impl::geometry_buffer::geometry_buffer(
    ...
    : log_{_log, sge::log::default_parameters(fcppt::log::name{FCPPT_TEXT("geometry_buffer")})},
      batches_(),
      renderer_(_renderer),
      vertex_declaration_(_vertex_declaration),
      active_texture_{},
      translation_(fcppt::math::vector::null<sge::renderer::vector3>()),
      pivot_(fcppt::math::vector::null<sge::renderer::vector3>()),
      rotation_(1.0F, 0.0F, 0.0F, 0.0F),
      scissor_area_(fcppt::math::box::null<sge::renderer::pixel_rect>()),
      render_context_(_render_context),
      clip_(true),
      render_effect_{},
      rasterizer_scissor_on_(
          renderer_.get().create_rasterizer_state(sge::cegui::impl::make_rasterizer_parameters(
              sge::renderer::state::core::rasterizer::enable_scissor_test(true)))),
      rasterizer_scissor_off_(
          renderer_.get().create_rasterizer_state(sge::cegui::impl::make_rasterizer_parameters(
              sge::renderer::state::core::rasterizer::enable_scissor_test(false)))){
          FCPPT_LOG_DEBUG(
              log_,
              fcppt::log::out << FCPPT_TEXT("geometry_buffer(") << this
                              << FCPPT_TEXT(")::geometry_buffer"))}

      sge::cegui::impl::geometry_buffer::~geometry_buffer() = default;
...
void sge::cegui::impl::geometry_buffer::draw() const
{
  ...
  sge::renderer::state::ffp::transform::object_unique_ptr const transform_state(
      renderer_.get().create_transform_state(sge::renderer::state::ffp::transform::parameters(
          fcppt::math::matrix::translation(translation_ + pivot_) *
          sge::cegui::impl::matrix_from_quaternion(rotation_) *
          fcppt::math::matrix::translation(-pivot_))));

  sge::renderer::state::ffp::transform::scoped const scoped_world(
      fcppt::make_ref(render_context),
      sge::renderer::state::ffp::transform::mode::world,
      fcppt::make_cref(*transform_state));

  if (this->getBlendMode() == CEGUI::BM_INVALID)
  {
    throw sge::cegui::exception{FCPPT_TEXT("Unexpected blend mode!")};
  }

  sge::renderer::state::core::blend::object_unique_ptr const blend_state(
      renderer_.get().create_blend_state(sge::renderer::state::core::blend::parameters(
          sge::renderer::state::core::blend::alpha_variant{
              sge::cegui::impl::to_blend_parameters(this->getBlendMode())},
          sge::renderer::state::core::blend::write_mask_all())));

  sge::renderer::state::core::blend::scoped const scoped_blend(
      fcppt::reference_to_base<sge::renderer::context::core>(fcppt::make_ref(render_context)),
      fcppt::make_cref(*blend_state));

  sge::renderer::target::base &current_target(render_context.target());

  sge::renderer::target::scoped_scissor_area const scoped_scissor_area(
      fcppt::make_ref(current_target), scissor_area_);

  sge::renderer::vertex::scoped_declaration const scoped_vdecl(
      fcppt::reference_to_base<sge::renderer::context::core>(fcppt::make_ref(render_context)),
      vertex_declaration_);

  int const pass_count(fcppt::optional::maybe(
      render_effect_,
      fcppt::const_(1),
      [](fcppt::reference<CEGUI::RenderEffect> const _effect)
      { return _effect.get().getPassCount(); }));

  sge::renderer::state::core::rasterizer::const_optional_object_ref prev_rasterizer{};

  for (int const pass : fcppt::make_int_range_count(pass_count))
  {
    fcppt::optional::maybe_void(
        render_effect_,
        [pass](fcppt::reference<CEGUI::RenderEffect> const _effect)
        { _effect.get().performPreRenderFunctions(pass); });

    for (sge::cegui::impl::batch const &batch : batches_)
    {
      sge::renderer::vertex::scoped_buffer const scoped_vb(
          fcppt::reference_to_base<sge::renderer::context::core>(fcppt::make_ref(render_context)),
          fcppt::make_cref(batch.vertex_buffer()));

      sge::renderer::texture::scoped const scoped_texture(
          fcppt::reference_to_base<sge::renderer::context::core>(fcppt::make_ref(render_context)),
          fcppt::reference_to_base<sge::renderer::texture::base const>(
              fcppt::make_cref(batch.texture())),
...

      {
        ...
        if (new_state != prev_rasterizer)
        {
          prev_rasterizer = new_state;

          render_context.rasterizer_state(new_state);
        }
      ...
      render_context.render_nonindexed(
          ...
          sge::renderer::vertex::count(batch.vertex_buffer().linear_size()),
...
void sge::cegui::impl::geometry_buffer::setTranslation(CEGUI::Vector3f const &_vec)
{
  translation_ = sge::cegui::impl::from_cegui_vector3(_vec);
}

void sge::cegui::impl::geometry_buffer::setRotation(CEGUI::Quaternion const &_rotation)
{
  rotation_ = _rotation;
}

void sge::cegui::impl::geometry_buffer::setPivot(CEGUI::Vector3f const &_vec)
{
  pivot_ = sge::cegui::impl::from_cegui_vector3(_vec);
}

void sge::cegui::impl::geometry_buffer::setClippingRegion(CEGUI::Rectf const &_rect)
{
  auto const converted(
      fcppt::math::box::structure_cast<sge::renderer::pixel_rect, fcppt::cast::float_to_int_fun>(
          sge::cegui::impl::from_cegui_rect(_rect)));

  FCPPT_LOG_DEBUG(
      log_,
      fcppt::log::out << FCPPT_TEXT("geometry_buffer(") << this
                      << FCPPT_TEXT(")::setClippingRegion(") << converted << FCPPT_TEXT(')'))

  scissor_area_ = sge::renderer::target::scissor_area{converted};
}

void sge::cegui::impl::geometry_buffer::appendVertex(CEGUI::Vertex const &_vertex)
{
    ...
    CEGUI::Vertex const *const _vertices, CEGUI::uint const _vertex_count)
{
  ...
  sge::renderer::vertex::scoped_lock const vblock(
      fcppt::make_ref(batches_.back().vertex_buffer()), sge::renderer::lock_mode::writeonly);

  sge::cegui::impl::vf::vertex_view const vertex_view(vblock.value());
  ...
  std::transform(
      ...
      {
        return sge::renderer::vf::vertex<sge::cegui::impl::vf::part>{
            sge::renderer::vf::labels::pos{} =
                sge::cegui::impl::from_cegui_vector3(_vertex.position),
            sge::renderer::vf::labels::texpos<0>{} =
                sge::cegui::impl::from_cegui_vector2(_vertex.tex_coords),
            sge::renderer::vf::labels::color{} = sge::image::color::rgba8(
...
void sge::cegui::impl::geometry_buffer::setActiveTexture(CEGUI::Texture *const _tex)
{
  if (_tex == nullptr)
  {
    throw sge::cegui::exception{FCPPT_TEXT("setActiveTexture with nullptr")};
  }

  active_texture_ = sge::cegui::impl::geometry_buffer::optional_texture_ref{
      fcppt::make_ref(dynamic_cast<sge::cegui::impl::texture &>(*_tex))};
}

void sge::cegui::impl::geometry_buffer::reset()
{
  batches_.clear();
...
CEGUI::Texture *sge::cegui::impl::geometry_buffer::getActiveTexture() const
{
  throw sge::cegui::exception{
      FCPPT_TEXT("The method is never called by anyone, so it's unimplemented here")};
}

CEGUI::uint sge::cegui::impl::geometry_buffer::getVertexCount() const
{
  throw sge::cegui::exception{
      FCPPT_TEXT("The method is never called by anyone, so it's unimplemented here")};
}

CEGUI::uint sge::cegui::impl::geometry_buffer::getBatchCount() const
{
  throw sge::cegui::exception{
      FCPPT_TEXT("The method is never called by anyone, so it's unimplemented here")};
}

void sge::cegui::impl::geometry_buffer::setRenderEffect(CEGUI::RenderEffect *const _render_effect)
{
  render_effect_ = fcppt::optional::from_pointer(_render_effect);
}

CEGUI::RenderEffect *sge::cegui::impl::geometry_buffer::getRenderEffect()
{
  return fcppt::optional::to_pointer(render_effect_);
}

void sge::cegui::impl::geometry_buffer::setClippingActive(bool const _active)
{
  clip_ = sge::cegui::impl::clip{_active};
}

bool sge::cegui::impl::geometry_buffer::isClippingActive() const { return clip_.get(); }
