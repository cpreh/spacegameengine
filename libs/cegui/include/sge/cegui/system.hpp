...
public:
  ...
  // durations and so on
  SGE_CEGUI_DETAIL_SYMBOL
  void update(sge::cegui::duration const &);

  SGE_CEGUI_DETAIL_SYMBOL
  void render(sge::renderer::context::ffp & // NOLINT(google-runtime-references)
  ); // NOLINT(google-runtime-references)

  [[nodiscard]] SGE_CEGUI_DETAIL_SYMBOL CEGUI::WindowManager &window_manager();

  [[nodiscard]] SGE_CEGUI_DETAIL_SYMBOL CEGUI::WindowManager const &window_manager() const;

  [[nodiscard]] SGE_CEGUI_DETAIL_SYMBOL CEGUI::GUIContext &gui_context();

  [[nodiscard]] SGE_CEGUI_DETAIL_SYMBOL CEGUI::GUIContext const &gui_context() const;

private:
  using system_impl_ptr = fcppt::unique_ptr<sge::cegui::detail::system_impl>;
