...
public:
  ...
  using file_unique_ptr = fcppt::unique_ptr<file>;
...
private:
  [[nodiscard]] load_stream_result
  mux_stream_try_all(sge::media::stream_unique_ptr &&, sge::media::optional_name const &) const;

  using plugin_type = sge::plugin::object<system>;

  using plugin_container = std::vector<plugin_type>;

  using system_unique_ptr = fcppt::unique_ptr<system>;

  using plugin_system_pair = std::pair<plugin_type, system_unique_ptr>;

  using plugin_system_pair_container = std::vector<plugin_system_pair>;
