...
class SGE_CORE_DETAIL_CLASS_SYMBOL raw_streambuf : public std::streambuf
{
  FCPPT_NONMOVABLE(raw_streambuf);

public:
  SGE_MEDIA_DETAIL_SYMBOL
  explicit raw_streambuf(sge::media::const_raw_range const &);

  SGE_MEDIA_DETAIL_SYMBOL
  ~raw_streambuf() override;
};
