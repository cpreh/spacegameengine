...
public:
  /**
	...
	*/
  [[nodiscard]] virtual sge::audio::sample_count
  // TODO(philipp): Return the buffer here?
  read(
      sge::audio::sample_count _samples,
      fcppt::reference<sge::audio::sample_buffer> _destination) = 0;

  /**
	\brief Read ALL the samples from the file.
	*/
  [[nodiscard]] virtual sge::audio::sample_container read_all() = 0;
  ...
  /**
	...
	*/
  [[nodiscard]] virtual sge::audio::sample_count expected_package_size() const = 0;

  /// How many channels does this file hold
  [[nodiscard]] virtual sge::audio::channel_count channels() const = 0;

  /// What's the sample rate (in number of samples) of the file
  [[nodiscard]] virtual sge::audio::sample_rate sample_rate() const = 0;

  /// How many bits per sample are there
  [[nodiscard]] virtual sge::audio::bits_per_sample bits_per_sample() const = 0;
