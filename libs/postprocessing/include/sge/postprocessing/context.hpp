...
public:
  ...
  context(sge::renderer::device::core_ref, sge::viewport::manager_ref, sge::shader::context_ref);

  SGE_POSTPROCESSING_DETAIL_SYMBOL
  sge::renderer::context::scoped_core_unique_ptr create_render_context();

  SGE_POSTPROCESSING_DETAIL_SYMBOL
  void render();

  SGE_POSTPROCESSING_DETAIL_SYMBOL
  sge::renderer::context::scoped_core_unique_ptr render_and_return_overlay();
...
private:
  ...
  using optional_planar_texture_ptr =
      fcppt::optional::object<sge::renderer::texture::planar_unique_ptr>;

  optional_planar_texture_ptr rendering_result_texture_;

  using optional_offscreen_target_ptr =
      fcppt::optional::object<sge::renderer::target::offscreen_unique_ptr>;

  optional_offscreen_target_ptr offscreen_target_;

  using optional_depth_stencil_surface_ptr =
      fcppt::optional::object<sge::renderer::depth_stencil_buffer::surface_unique_ptr>;
  ...
  void switch_target_texture(sge::renderer::texture::planar_ref);
  ...
  [[nodiscard]] sge::renderer::context::scoped_core_unique_ptr finalize();
};
