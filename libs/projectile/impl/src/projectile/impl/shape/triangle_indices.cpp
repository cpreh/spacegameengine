    ...
    fcppt::log::object &_log, sge::projectile::shape::triangle_sequence const &_triangles)
{
  ...
  FCPPT_LOG_DEBUG(
      _log, fcppt::log::out << FCPPT_TEXT("filled scalar container, now filling index container"))

  auto current_index_iterator(indices.begin());

  sge::projectile::shape::detail::index_container::value_type current_index(0);
  ...
  FCPPT_LOG_DEBUG(
      _log,
      fcppt::log::out << FCPPT_TEXT("filled index container (last index was ") << current_index
