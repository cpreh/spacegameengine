...
std::filesystem::path sge::config::app_path()
{
  ...
  buffer_type const buf{fcppt::container::buffer::to_raw_vector(
      fcppt::container::buffer::read_from<fcppt::container::buffer::object<fcppt::char_type>>(
          32768u,
          [](buffer_type::pointer const _data, buffer_type::size_type const _size)
          {
            DWORD const count{::GetModuleFileName(NULL, _data, fcppt::cast::size<DWORD>(_size))};

            if (count == 0u)
            {
              throw sge::config::exception{FCPPT_TEXT("GetModuleFileName() failed!")};
            }
  ...
  {
    throw sge::config::exception{FCPPT_TEXT("/prof/self/exe does not exist")};
  }

  return std::filesystem::read_symlink(self).parent_path();
#else
#error "Implement me!"
#endif
}
