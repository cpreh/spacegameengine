...
public:
  ...
  explicit context(sge::plugin::context_base_ref);
  ...
  using object = sge::plugin::object<Type>;

  [[nodiscard]] SGE_PLUGIN_DETAIL_INSTANTIATE_SYMBOL object load() const;

  [[nodiscard]] SGE_PLUGIN_DETAIL_INSTANTIATE_SYMBOL std::filesystem::path const &path() const;

  [[nodiscard]] SGE_PLUGIN_DETAIL_INSTANTIATE_SYMBOL sge::plugin::info const &info() const;
