...
#define SGE_PLUGIN_LIBRARY_MAKE_INTERFACE(plugin_info, plugin_functions) \
  namespace \
  { \
\
  sge::plugin::info sge_info_function() { return plugin_info; } \
  } \
\
  ...
  sge::plugin::library::function_map::container{ \
      sge::plugin::library::detail::make_version_function(&sge_info_function) \
          BOOST_PP_SEQ_FOR_EACH( \
              SGE_PLUGIN_LIBRARY_DETAIL_ITERATE_FUNCTIONS, \
              , \
              plugin_functions)} SGE_PLUGIN_LIBRARY_DETAIL_INTERFACE_POST FCPPT_PP_POP_WARNING
