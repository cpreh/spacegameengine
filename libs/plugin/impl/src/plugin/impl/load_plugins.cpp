...
sge::plugin::detail::context_base_vector sge::plugin::impl::load_plugins(
    fcppt::log::object &_log,
    std::filesystem::path const &_path,
    sge::plugin::optional_cache_ref const &_cache)
{
  return fcppt::algorithm::map_optional<sge::plugin::detail::context_base_vector>(
      fcppt::either::to_exception(
          fcppt::filesystem::make_directory_range(_path, std::filesystem::directory_options::none),
          [](std::error_code const _error)
          {
            return sge::plugin::exception{
                FCPPT_TEXT("Cannot access plugin directory: ") +
                fcppt::error_code_to_string(_error)};
          }),
      [&_log, &_cache](std::filesystem::path const &_cur_path)
      {
        using optional_result = fcppt::optional::object<sge::plugin::context_base_unique_ptr>;

        if (std::filesystem::is_directory(_cur_path) ||
            fcppt::filesystem::extension_without_dot(_cur_path) != sge::plugin::file_extension())
        {
          FCPPT_LOG_WARNING(
              _log,
        ...
        catch (sge::core::exception const &_exception)
        {
          FCPPT_LOG_WARNING(
              _log,
              fcppt::log::out << fcppt::filesystem::path_to_string(_cur_path)
                              << FCPPT_TEXT(" failed to load: \"") << _exception.string()
                              << FCPPT_TEXT("\"!"))
        }
