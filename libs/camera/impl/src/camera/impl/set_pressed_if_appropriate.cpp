...
void sge::camera::impl::set_pressed_if_appropriate(
    fcppt::reference<bool> const _result,
    sge::input::key::optional_code const &_optional_key,
    sge::input::keyboard::event::key const &_key_event)
{
  fcppt::optional::maybe_void(
      _optional_key,
      [&_key_event, &_result](sge::input::key::code const _code)
      {
        if (_code == _key_event.get().code())
        {
          _result.get() = _key_event.pressed();
        }
      });
}
