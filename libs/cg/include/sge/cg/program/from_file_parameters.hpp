...
#include <sge/cg/program/from_file_parameters_fwd.hpp> // IWYU pragma: keep
...
public:
  ...
  [[nodiscard]] sge::cg::context::object const &context() const;

  [[nodiscard]] sge::cg::program::source_type source_type() const;

  [[nodiscard]] sge::cg::profile::object const &profile() const;

  [[nodiscard]] std::filesystem::path const &path() const;

  [[nodiscard]] sge::cg::program::main_function const &main_function() const;

  [[nodiscard]] sge::cg::program::compile_options const &compile_options() const;

private:
  fcppt::reference<sge::cg::context::object const> context_;

  sge::cg::program::source_type source_type_;

  fcppt::reference<sge::cg::profile::object const> profile_;
