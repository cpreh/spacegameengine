...
class options
{
  using elements_type = sge::sprite::state::detail::options_class<StateChoices>;

public:
  explicit options(sge::sprite::state::vertex_options);

  options &no_blend_state();

  options &no_rasterizer_state();

  options &no_transform_state();

  options &fixed_projection(sge::sprite::projection_dim const &);

  [[nodiscard]] sge::sprite::state::vertex_options vertex_options() const;

  template <typename Role>
  options &set(fcppt::record::label_value_type<elements_type, Role>);
