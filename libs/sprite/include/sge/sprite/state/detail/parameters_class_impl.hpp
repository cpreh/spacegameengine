...
private:
  ...
  using has_parameter = std::conjunction<typename Type::persistent, typename Type::has_parameter>;

  template <typename Type>
  using parameter_class_element = fcppt::record::
      element<typename Type::parameter_role, typename Type::optional_extra_parameters>;

public:
  using type = fcppt::record::from_list<fcppt::mpl::list::map<
      fcppt::mpl::list::
          keep_if<typename StateChoices::optional_elements, fcppt::mpl::lambda<has_parameter>>,
      fcppt::mpl::lambda<parameter_class_element>>>;
};
