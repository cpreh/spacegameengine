...
template <typename Level, typename Choices>
inline bool texture_level_equal(
    sge::sprite::object<Choices> const &_left, sge::sprite::object<Choices> const &_right)
{
  static_assert(
      sge::sprite::detail::config::has_texture<Choices>::value,
      "Sprite choices must have a texture");

  return sge::sprite::compare::detail::texture_level_function<std::equal_to, Level>::execute(
      _left, _right);
}
