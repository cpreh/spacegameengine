...
sge::sprite::intrusive::ordered::iterator<Choices, Order, IsConst>::iterator(
    ...
    : outer_iterator_(
          ...
          _outer_iterator_end),
      inner_iterator_(
          this->is_end() ? range_iterator() : outer_iterator_.base()->second->range().begin())
{
}
...
template <typename Choices, typename Order, bool IsConst>
void sge::sprite::intrusive::ordered::iterator<Choices, Order, IsConst>::increment()
{
  ++inner_iterator_;

  if (inner_iterator_ == outer_iterator_.base()->second->range().end())
  {
    ++outer_iterator_;

    if (!this->is_end())
    {
      inner_iterator_ = outer_iterator_.base()->second->range().begin();
    }
  }
}
...
template <typename Choices, typename Order, bool IsConst>
bool sge::sprite::intrusive::ordered::iterator<Choices, Order, IsConst>::equal(
    iterator const &_other) const
{
  return outer_iterator_ == _other.outer_iterator_ &&
         ((this->is_end() && _other.is_end()) || (inner_iterator_ == _other.inner_iterator_));
}

template <typename Choices, typename Order, bool IsConst>
bool sge::sprite::intrusive::ordered::iterator<Choices, Order, IsConst>::is_end() const
{
  return outer_iterator_.base() == outer_iterator_.end();
}
