...
inline sge::sprite::count
range_size(sge::sprite::intrusive::ordered::detail::map<Order, Choices> const &_ordered_map)
{
  return fcppt::algorithm::fold(
      _ordered_map,
      sge::sprite::count{0U},
      [](auto const &_element, sge::sprite::count const _value)
      { return _value + _element.second->range().size(); });
}
