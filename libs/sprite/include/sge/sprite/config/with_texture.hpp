...
struct with_texture<sge::sprite::config::texture_level_count<TextureLevels>, Coordinates, Ownership>
{
  static_assert(TextureLevels >= 1U, "with_texture requires at least one texture level");

  using texture_levels = sge::sprite::config::texture_level_count<TextureLevels>;

  using coordinates = std::integral_constant<sge::sprite::config::texture_coordinates, Coordinates>;

  using ownership = std::integral_constant<sge::sprite::config::texture_ownership, Ownership>;

  template <typename Choices>
  using apply =
      sge::sprite::detail::primitives::texture<Choices, texture_levels, Coordinates, Ownership>;
};
