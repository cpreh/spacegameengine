...
void fill_indices(
    sge::sprite::count const _sprite_count, sge::sprite::buffers::slice<Choices> const &_slice)
{
  sge::renderer::index::scoped_lock const lock(
      fcppt::make_ref(_slice.index_buffer()),
      sge::renderer::lock_segment{
          sge::renderer::lock_segment::vector{_slice.first_index().get()},
          sge::renderer::lock_segment::dim{
              sge::sprite::buffers::index_count<Choices>(_sprite_count).get()}},
      sge::renderer::lock_mode::writeonly);

  sge::renderer::index::any::generate(
      sge::renderer::index::any::make_view(lock.value()),
      sge::sprite::geometry::detail::index_generator<Choices>());
}
