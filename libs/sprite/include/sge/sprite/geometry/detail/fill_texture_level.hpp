...
public:
  ...
  fill_texture_level(Iterator const &_iterator, object const &_object)
      : iterator_(_iterator), object_(_object)
  {
  }
  ...
  {
    using level = fcppt::tag_type<Level>;

    sge::sprite::geometry::detail::fill_texture_level_impl(
        level{},
        iterator_,
        object_.get(),
        sge::sprite::deref_texture(object_.get().template texture_level<level::value>()));
  }

private:
  Iterator iterator_;

  fcppt::reference<object const> object_;
};
