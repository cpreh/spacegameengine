...
sge::timer::clocks::adjustable<ClockImpl>::adjustable()
    : current_time_(ClockImpl::now()), transformed_time_(current_time_), factor_(1.0f)
{
}
...
template <typename ClockImpl>
void sge::timer::clocks::adjustable<ClockImpl>::update()
{
  ...
  time_point const latest_time = ClockImpl::now();

  duration const diff(
      static_cast<rep>(factor_ * static_cast<float_type>((latest_time - current_time_).count())));

  transformed_time_ += diff;

  current_time_ = latest_time;
}

template <typename ClockImpl>
typename sge::timer::clocks::adjustable<ClockImpl>::float_type
...
template <typename ClockImpl>
void sge::timer::clocks::adjustable<ClockImpl>::factor(float_type const _factor)
{
  factor_ = _factor;
}

template <typename ClockImpl>
typename sge::timer::clocks::adjustable<ClockImpl>::time_point
