...
sge::timer::absolute<Clock>::absolute() : absolute(clock_parameter{})
{
...
template <typename Clock>
sge::timer::absolute<Clock>::absolute(clock_parameter const _clock)
    : base(_clock), last_time_{this->now()}
{
}
...
{
template <typename Clock>
absolute<Clock>::~absolute() = default;
}
...
template <typename Clock>
void sge::timer::absolute<Clock>::reset()
{
  last_time_ = this->now();
}
