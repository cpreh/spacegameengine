...
namespace sge
{
	namespace ogl
	{
		...
		public:
			texture(const const_pointer src, const size_type width, const size_type height, const unsigned mipmaps, const resource_flag_t flags)
				: basic_texture<sge::texture,GL_TEXTURE_2D>(width*height,mipmaps,flags), width(width), height(height)
			{
				set_data(src);
			}
			...
			{
				bind_me();
				const GLenum format = GL_RGBA, type = GL_UNSIGNED_BYTE; // GL_UNSIGNED_INT_8_8_8_8;
				if(!r)
