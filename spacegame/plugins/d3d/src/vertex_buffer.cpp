...
sge::d3d::vertex_buffer::vertex_buffer(renderer* const r, d3d_device_ptr device,
									   const sge::vertex_format& format, const size_type sz,
									   const resource_flag_t flags, const const_pointer src)
: resource(r),
  device(device), lock_dest(0), flags(flags), sz(sz), format(format), d3d_format(device,format)
{
	init(src);
}
...
void sge::d3d::vertex_buffer::init(const const_pointer src)
{
	...
	if(src)
	{
		...
		copy(src,src+sz*get_stride(),lock_dest);
    }
}
...
sge::d3d::vertex_buffer::iterator sge::d3d::vertex_buffer::create_iterator(pointer data) const
{
	return iterator(data,d3d_format.get_stride(),d3d_format.get_offset_info());
}

sge::d3d::vertex_buffer::const_iterator sge::d3d::vertex_buffer::create_iterator(const_pointer data) const
{
...
void sge::d3d::vertex_buffer::lock(const lock_flag_t flags, const size_type first, const size_type count)
{
	...
	if(buffer->Lock(first*get_stride(),count*get_stride(),&p,lflags) != D3D_OK)
		throw std::runtime_error("cannot lock vertex buffer");
...
void sge::d3d::vertex_buffer::unlock()
{
	...
	if(buffer->Unlock() != D3D_OK)
		throw std::runtime_error("Cannot unlock vertex buffer");
	lock_dest = 0;
}

void sge::d3d::vertex_buffer::resize(size_type newsize)
{
	if(lock_dest)
...
void sge::d3d::vertex_buffer::restore()
{
	init();
}

sge::d3d::vertex_buffer::size_type sge::d3d::vertex_buffer::size() const { return sz; }
sge::vertex_size sge::d3d::vertex_buffer::get_stride() const { return d3d_format.get_stride(); }
sge::resource_flag_t sge::d3d::vertex_buffer::get_flags() const { return flags; }
sge::d3d::vertex_buffer::pointer sge::d3d::vertex_buffer::raw_data() { return lock_dest; }
sge::d3d::vertex_buffer::const_pointer sge::d3d::vertex_buffer::raw_data() const { return lock_dest; }
const sge::vertex_format& sge::d3d::vertex_buffer::get_vertex_format() const { return format; }
