...
namespace sge
{
	namespace d3d
	{
		...
		void add_display_modes(display_mode_array& v, unsigned adapter,
			                   bit_depth depth, D3DFORMAT format, d3d_ptr sys);
		D3DFORMAT search_format(const display_mode& mode, d3d_ptr sys );
		D3DFORMAT search_stencil_format(unsigned adapter, D3DFORMAT screen_format,
			                            d3d_ptr sys);
		D3DPRESENT_PARAMETERS create_present_parameters(const renderer_parameters& param,
			                                            int adapter, win32_window_ptr wnd,
														d3d_ptr sys);
	}
}
