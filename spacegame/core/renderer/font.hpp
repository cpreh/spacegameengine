...
public:
	...
	font(renderer_ptr r, font_system_ptr font_sys, const std::string& font_name, bool italic = false, font_weight weight = FW_Normal, text_unit height = 0.1f, const std::locale& loc = std::locale());
	...
	text_size char_size(char_type ch) const;
	text_size string_size(string_type::const_iterator beg, string_type::const_iterator end) const;
	text_size string_size(const string_type& s, string_type::size_type pos = 0, string_type::size_type count = string_type::npos) const;
	text_size string_size(const string_type& text, text_unit width, text_flag_t flags = TXTF_AlignLeft | TXTF_AlignTop) const;
