...
sge::gui::text_edit::text_edit(manager& m, element* const parent, const point pos, const dim sz, const std::string& _text, const color text_color, const bool visible, const bool enabled)
...
void sge::gui::text_edit::on_draw(const draw_event& event)
{
	...
	my_font.draw_text(_text.substr(text_start,_text.size()-text_start),event.pos(),size(),text_color,TXTF_AlignLeft | TXTF_AlignVCenter | TXTF_NoMultiLine);
	if(m.focus() == this)
	{
		point pos = event.pos();
