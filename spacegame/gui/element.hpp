...
namespace gui
{
	...
	public:
		SGEGUIDLLAPI virtual ~element();
		SGEGUIDLLAPI virtual void attach_child(element* element);
		SGEGUIDLLAPI virtual void detach_child(element* element);

		SGEGUIDLLAPI void set_parent(element* element);
		SGEGUIDLLAPI element* get_parent() const;
		SGEGUIDLLAPI void show();
		SGEGUIDLLAPI void hide();
		SGEGUIDLLAPI bool is_visible() const;
		SGEGUIDLLAPI void set_enable(bool enabled);
		SGEGUIDLLAPI bool get_enable() const;
		SGEGUIDLLAPI void to_front();
		SGEGUIDLLAPI bool pressed() const;
		...
		signal_type mouse_move_signal,
			    ...
			    key_press_signal;
	protected:
		SGEGUIDLLAPI element(manager& m, element* parent, bool visible = true, bool enabled = true);
		manager& m;
	private:
